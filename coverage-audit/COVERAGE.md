...
| 1.3 | ~~**Header self-derivation still bootstrapped**~~ **CLOSED 2026-09-02, EXTENDED 2026-09-03** for the standalone entry point: `aom_encode::key_frame::encode_key_frame` derives base_qindex (`rc::base_qindex_from_cq`), the tile config (`av1_get_tile_limits` + `av1_calculate_tile_cols/rows`), the CICP/colour echo, the loop-filter levels, the `tx_mode` SELECT->LARGEST flip (a new `txb_split_count` over the port's own trees) and the full temporal-unit / TD-OBU self-assembly. **225/225** cells byte-identical to real aomenc (up from 69/69, then 186/186; 2026-09-03 added bd10/12 x speed x tile-count coverage, a historical-poison-geometry regression probe, and two adversarial probes for the still-unported `av1_determine_sc_tools_with_encoding` — see `CLAUDE.md`'s coverage queue and `STATUS.md`'s 2026-09-03 entry for the measured bands), both decoders agreeing (`aom-encode/tests/self_contained_key_frame.rs`). 7 pins remain, all with a measured `Where` attribution. `aom-bench`'s `port_encode*` still takes a bootstrap by signature — a separate entry point, unchanged. | `bitstream.c write_uncompressed_header_obu` | **DONE** (standalone path) / PARTIAL (aom-bench) | done | this landing |
