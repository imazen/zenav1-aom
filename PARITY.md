...
| **SELF-CONTAINED KEY-FRAME ENCODE — no C bootstrap anywhere in the path** (`aom_encode::key_frame::encode_key_frame`): the port AUTHORS its own TD OBU + sequence-header OBU + frame header and packs its own tile. **427/427 cells BYTE-IDENTICAL to real aomenc's whole temporal unit** (186/186 on 2026-09-02; 241/241 after the SB128 + explicit-tile landings on 2026-09-03; +186 coded-lossless cells on 2026-09-04) — cq 0..63 step 5 (+63), a 1..19 step-2 low-q arm, a dense **cq-0 / coded-lossless N arm** (189 cq-0 cells in all) ({mono, 4:2:0, 4:2:2, 4:4:4} x bd {8,10,12} x 5 contents x `--cpu-used` {0,9}, + bd8 at {3,6}, + a 1x1..258x258 size ladder), {mono, 4:2:0, 4:2:2, 4:4:4} x bd {8, 10, 12} (profiles 0/1/2), sizes 16x16..512x512, 12 crop / partial-SB sizes incl. 1x1, 5 content classes (flat / gradient / texture / noise / checkerboard), **all four (CDEF, loop-restoration) combinations including BOTH ON** (27 post-filter cells across mono/4:2:0/4:4:4 x cq 5..63 x 64x64..256x256), **`--cpu-used` 0..=9** (68 speed cells), and **MANDATORY multi-tile** (frames wider than `MAX_TILE_WIDTH`: 4160x64 / 4224x128 / 4160x192 two-tile and 8320x64 three-tile, 8192x64 four-tile, speeds 0..6 -- each tile packed independently with a fresh frame context, assembled through `assemble_multitile_frame_obu_payload_derived`). The tile-count derivation models `set_tile_info`'s STRICTER column minimum (`encoder.c:386-390` uses `(max_width_sb << k) <= sb_cols`, one more than `av1_get_tile_limits`' own `tile_log2`, which differ exactly at a frame whose `sb_cols` is an exact multiple-by-power-of-two of `max_width_sb`); bite-proved by weakening it to `<`, which makes 4033x64, 4096x64 and 8192x64 diverge while 4032x64 / 4097x64 / 4160x64 / 2048x64 stay exact. Real aomenc's ALLINTRA default is CDEF OFF with restoration ON (`av1_cx_iface.c:3067`) and is byte-gated at EVERY speed; `--enable-cdef=1` is byte-gated at speeds 0..3 and pinned-divergent at 4..9 (the FAST search levels, PARITY C1's never-e2e-gated fraction, measured divergent here in the header's `cdef_strengths` only). The port also models `speed_features.c:2753`, which CLEARS the sequence header's `enable_restoration` bit at allintra speed >= 5 -- without it every `--enable-restoration=1` cell at speed 5..9 diverged. Both the real C decoder AND `aom-decode` decode the PORT's own stream to the pixels real aomenc's stream decodes to. Derived, not replayed: `seq_level_idx`/`tier` (`set_bitstream_level_tier`), profile / bit depth / subsampling / `num_bits_*` / the reduced-still-picture framing, `base_qindex`, `allow_screen_content_tools` (the ported detector DRIVES it now), the tile grid, the loop-filter levels, and the `tx_mode` SELECT->LARGEST flip via a new `txb_split_count` over the port's own winner trees. Post-filter order is C's `cdef_restoration_frame`: deblock -> `av1_cdef_search` -> `av1_cdef_frame` (apply) -> `av1_pick_filter_restoration` on the POST-CDEF recon, packed through the new additive `pack::pack_tile_from_trees_lr` (neither predecessor carried CDEF literals AND per-RU restoration params, so the default config had no pack entry point). Envelope: ALLINTRA, `--cpu-used` 0..=9, SB64 **and SB128** (SB128 wired 2026-09-03), single tile and explicitly-requested multi-tile — everything else REFUSED by name (`KeyFrameError`). **The same landing found and fixed a real shell bug the gate had mis-attributed:** the pack env carried a past-the-end sentinel for `tile_row_end`/`tile_col_end` instead of C's `AOMMIN(.., mi_rows/mi_cols)` clamp (`av1_tile_set_row`/`_col`), which changes the search's frame-edge decisions. Reverting the clamp makes 131x131, 132x64, 132x128, 132x132, 196x64, 196x196, 260x260 and 261x261 all diverge and the clamp makes all eight byte-identical — bite-proved. Four of those had been pinned as "RD near-ties"; they were this. Also fixed a real bug in BOTH screen-detector callers: C reads the 8-ALIGNED `y_width`/`y_height`, not the crop. | `self_contained_key_frame` (`byte_matches_real_aomenc` 427/427, `decodes_to_the_same_pixels` 30 cells x 2 decoders, `coded_lossless_reconstructs_the_source_exactly` 248/248 x 2 decoders, `no_seq_header_stream_is_rejected_by_the_c_decoder`, `mutated_sequence_header_is_caught`, `refuses_configurations_it_has_no_gate_for`, `open_divergences_are_pinned` 9 pins) + `seq_level_idx_diff` | (2026-09-02, extended 2026-09-03 and 2026-09-04) |
| Gate-3 perf cells byte-verified before timing | `aom-bench` `EncodeCell::assert_byte_exact` | 057bde2 |
| **SCREEN-CONTENT TOOLS ON THE SHIPPING PATH — palette + IntraBC wired into `encode_key_frame`, against a MATCHED oracle** (2026-09-10). The shell built its `PickFrameCfg` with `palette_costs: None` / `intrabc: None`, so it ran NEITHER search on ANY frame — including frames whose header it writes with `allow_screen_content_tools = 1`. **The 427-cell gate could not see it: its oracle `shim_encode_av1_kf` hardcodes `enable_palette = 0, enable_intrabc = 0` (`dec_shim.c:612`), so that count is parity against a palette-DISABLED libaom.** Cost of the gap, libaom on both sides: **−73.1 % bytes at 1024x768 cq20 s3** (`benchmarks/encoder_screen_tools_2026-09-10.md`). **54/54 byte-identical** to `shim_encode_av1_kf_screen_content` driven with the SAME knobs (3 sizes x 3 quantizers x 3 speeds x checkerboard/UI), each cell asserting non-vacuity (the oracle's tools-ON stream must differ from its own tools-OFF stream) and a real-C decode round-trip; **24/24** in the wider `screen_tools_gap` probe. **The gate found a stream-corruption bug on its first run**: `uncompressed_header` skips loop_filter/cdef/lr params when `allow_intrabc` is set and the three sub-header structs each carry the bit, which `derive_frame_header` hardcoded false — three syntax elements the decoder never reads, desynchronising the tile group in the same OBU_FRAME. Constant +3 bytes over a byte-IDENTICAL tile payload. **Update 2026-09-13 (KB-65):** the coded-lossless IntraBC arm is now PORTED — `av1_txfm_search` dispatches to `av1_pick_uniform_tx_size_type_yrd` -> `choose_smallest_tx_size` (flat TX_4X4 + FWHT, `predict_skip_txfm` gated `!lossless`); the port's `var_tx::choose_smallest_tx_size_inter` mirrors it. The landing fixed two further roots the arm exposed: the write-side walks (`ibc_encode_block_inter_y`, `pack_vartx_txb`) rooted their recursion at `MAX_TXSIZE_RECT_LOOKUP` where `get_vartx_max_txsize` (blockd.h:1452) returns TX_4X4 at lossless — quadtree-DFS txbs into a raster-reading decoder = NON-CONFORMANT streams (`Invalid intrabc dv`) — and `search_allow_intrabc` missed the `& enable_intrabc` term (encodeframe.c:2194), a +51/leaf phantom flag charge in knob-off screen encodes. 48-cell probe matrix (`ibc_lossless_probe`): **47/48 byte-exact, every stream conformant on both real decoders**; residual = `Screen 512x384 cq0 s0` +3 B (0.01 %), a recon-identical filter-intra-vs-PAETH rate near-tie — measured, attributed, bounded. | `self_contained_key_frame::screen_content_tools_byte_match_real_aomenc` + `aom-bench/examples/screen_tools_gap.rs` | 2026-09-10 |
...
| C9 `--use-intra-dct-only=1` (CLOSED 2026-09-13, KB-60: all grid cells byte-identical) | `AV1E_SET_INTRA_DCT_ONLY=1` | 64²cq32 / 64²cq63 / 128²cq12 (real content) | EXACT / EXACT / EXACT | 0 / 0 / 0 | `toggles_rd_close::toggles_c9_intra_dct_only` | 2026-07-17 → 2026-09-13 | Root cause (KB-60): under `use_intra_dct_only` C's `get_tx_mask` pins the chroma FORWARD/quant search to DCT_DCT, but `dist_block_px_domain` and `inverse_transform_block_facade` re-derive the chroma type via `av1_get_tx_type` — the UV-MODE-derived type (DCT_ADST for H at TX_8x16; demoted to DCT at 32x32, which is why the bs9 evals had matched). C forward-transforms DCT coefficients but inverts with the mode-derived transform — decode-consistent, since the decoder derives chroma tx from `uv_mode` too. For uv=H the port's dist was 78,000 vs C's 1,694,016 (same residual/coeffs/rate), so the port wrongly admitted H where C bailed INVALID. Fix: `tx_search.rs` px-domain dist + `calc_pixel_domain_distortion_final` and `intra_uv_rd.rs`'s committed inverse now use `uv_intra_tx_type(...)` for chroma. |
...
### C9 — Transform controls — DONE (byte-exact); dct-only CLOSED by KB-60
...
- **`--use-intra-dct-only=1` — CLOSED 2026-09-13 (KB-60).** Under
  `use_intra_dct_only` C pins the chroma FORWARD search to DCT (`get_tx_mask`)
  but measures pixel-domain distortion and reconstructs with the UV-mode-derived
  type (`av1_get_tx_type`); the port applied the searched type. Fixed in
  `tx_search.rs` + `intra_uv_rd.rs`; all three grid cells byte-identical, the
  coding-tools gate is 48/48 and the dct-only combination array is 0/17
  divergent.
...
- **DONE 2026-09-04 — the CHROMA-FORMAT and CONTENT axes, plus the zenavif#45 root.**
  `self_contained_key_frame`'s new N arm sweeps cq 0 over {mono, 4:2:0, **4:2:2**,
  **4:4:4**} x bd {8, 10, 12} x 5 content classes x `--cpu-used` {0, 9} (+ bd8 at
  {3, 6}) and a 13-point size ladder 1x1..258x258 — **186 new cells, all
  byte-identical to real aomenc** (gate 241/241 -> **427/427**, re-measured on the
  merged tree; the gate's cq-0 population goes 3 -> 189). A second test,
