//! Repro (aarch64 only): port `aom_quantize_b_no_qmatrix` vs the REAL C NEON
//! `ref_quantize_b_neon`, which exists only in an aarch64 `aom-sys-ref`.
...
mod bench {
    ...
    pub fn main() {
        ...
        let mut mism_neon = 0u32;
        let mut mism_scalar = 0u32;
        let mut total = 0u32;
        ...
        println!(
            "total={total} port-vs-CNEON mism={mism_neon} CNEON-vs-Cscalar mism={mism_scalar}"
        );
    }
}
...
fn main() {
    eprintln!("qb_neon_repro: aarch64-only (needs the NEON C shim); nothing to run here");
}
