...
mod imp {
    ...
    // ---- widen / unsigned-pack twins -----------------------------------------
    //
    // Needed by the i16 fused kernels' u8 epilogues (`clip_pixel`):
    // `cvtepu8` is the load-side zero-extend, `packus` the store-side
    // unsigned saturate.

    /// `pmovzxbd`-class widen — the low 8 bytes, zero-extended to u16 lanes.
    #[inline]
    #[target_feature(enable = "neon")]
    pub fn _mm_cvtepu8_epi16(a: __m128i) -> __m128i {
        backu16(vmovl_u8(vget_low_u8(u8v(a))))
    }
    /// `packuswb` — unsigned-saturating i16 -> u8 narrow, `a` into the low
    /// half. `vqmovun_s16` is the same `clamp(_, 0, 255)` edge semantics.
    #[inline]
    #[target_feature(enable = "neon")]
    pub fn _mm_packus_epi16(a: __m128i, b: __m128i) -> __m128i {
        backu8(vcombine_u8(vqmovun_s16(s16(a)), vqmovun_s16(s16(b))))
    }
    /// `vpmovzxbw` ymm — zero-extend all 16 bytes to u16 lanes across the
    /// two 128-bit halves.
    #[inline]
    #[target_feature(enable = "neon")]
    pub fn _mm256_cvtepu8_epi16(a: __m128i) -> __m256i {
        [
            backu16(vmovl_u8(vget_low_u8(u8v(a)))),
            backu16(vmovl_u8(vget_high_u8(u8v(a)))),
        ]
    }
    /// `vpackuswb` ymm — per-128-lane unsigned-saturating narrow, matching
    /// the AVX2 lane-local semantics.
    #[inline]
    #[target_feature(enable = "neon")]
    pub fn _mm256_packus_epi16(a: __m256i, b: __m256i) -> __m256i {
        [_mm_packus_epi16(a[0], b[0]), _mm_packus_epi16(a[1], b[1])]
    }

    // ---- ymm twins used by the lane-doubled inverse-transform bodies ---------
    //
    // Every one is per-128-half application of the xmm twin — the AVX2 ops
    // are all elementwise or lane-local, so the decomposition is exact by
    // construction.
    ...
    pub fn _mm256_set1_epi32(x: i32) -> __m256i {
        [vdupq_n_s32(x), vdupq_n_s32(x)]
    }
    /// `_mm256_set_m128i(hi, lo)` — the x86 `set` order (high half first).
    #[inline]
    #[target_feature(enable = "neon")]
    ...
    pub fn _mm256_subs_epi16(a: __m256i, b: __m256i) -> __m256i {
        [
            back16(vqsubq_s16(s16(a[0]), s16(b[0]))),
            back16(vqsubq_s16(s16(a[1]), s16(b[1]))),
        ]
    }
    /// `vpmaddwd` ymm.
    #[inline]
    ...
    pub fn _mm256_madd_epi16(a: __m256i, b: __m256i) -> __m256i {
        [_mm_madd_epi16(a[0], b[0]), _mm_madd_epi16(a[1], b[1])]
    }
    /// `vpmulhrsw` ymm.
    #[inline]
    #[target_feature(enable = "neon")]
    pub fn _mm256_mulhrs_epi16(a: __m256i, b: __m256i) -> __m256i {
        [_mm_mulhrs_epi16(a[0], b[0]), _mm_mulhrs_epi16(a[1], b[1])]
    }
    /// `vpsrad` ymm with a runtime count in `count`'s low i64.
    #[inline]
    #[target_feature(enable = "neon")]
    pub fn _mm256_sra_epi32(a: __m256i, count: __m128i) -> __m256i {
        [_mm_sra_epi32(a[0], count), _mm_sra_epi32(a[1], count)]
    }
    /// `vpshufd` ymm — the same i32-lane permutation per 128-bit half.
    #[inline]
    #[target_feature(enable = "neon")]
