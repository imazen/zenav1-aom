...
fn run_inv1d(
    ...
    match k {
        ...
        Inv1d::Dct16 => incant!(
            av1_idct16_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Inv1d::Dct32 => incant!(
            av1_idct32_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Inv1d::Dct64 => incant!(
            av1_idct64_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Inv1d::Adst4 => incant!(
            av1_iadst4_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Inv1d::Adst8 => incant!(
            av1_iadst8_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Inv1d::Adst16 => incant!(
            av1_iadst16_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Inv1d::Idtx4 => incant!(
            av1_iidentity4_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Inv1d::Idtx8 => incant!(
            av1_iidentity8_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Inv1d::Idtx16 => incant!(
            av1_iidentity16_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Inv1d::Idtx32 => incant!(
            av1_iidentity32_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
    }
}
...
fn run_fwd1d(
    ...
    match k {
        ...
        Fwd1d::Dct16 => incant!(
            av1_fdct16_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Fwd1d::Dct32 => incant!(
            av1_fdct32_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Fwd1d::Dct64 => incant!(
            av1_fdct64_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Fwd1d::Adst4 => incant!(
            av1_fadst4_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Fwd1d::Adst8 => incant!(
            av1_fadst8_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Fwd1d::Adst16 => incant!(
            av1_fadst16_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Fwd1d::Idtx4 => incant!(
            av1_fidentity4_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Fwd1d::Idtx8 => incant!(
            av1_fidentity8_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Fwd1d::Idtx16 => incant!(
            av1_fidentity16_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
        Fwd1d::Idtx32 => incant!(
            av1_fidentity32_impl(input, out, cos_bit, stage_range),
            [v3, neon]
        ),
    }
}
...
fn inv_rect48_fused(
    ...
    incant!(
        run_inv1d(kr, &k[..col_n], &mut w[..col_n], cos_bit, sr_row),
        [v3, neon]
    );
    ...
    for r in 0..row_n {
        ...
        let v = if lr_flip {
            if col_n == 8 {
                ...
                i32x8::from_array(
                    t,
                    core::array::from_fn(|j| if j < col_n { a[col_n - 1 - j] } else { 0 }),
                )
            }
        } else {
            tr[r]
    ...
    incant!(
        run_inv1d(kc, &ci[..row_n], &mut co[..row_n], cos_bit, sr_col),
        [v3, neon]
    );
    ...
    for r in 0..row_n {
        ...
        if col_n == 8 {
            ...
            let dv = i32x8::from_array(
                t,
                core::array::from_fn(|j| if j < 4 { d[j] as i32 } else { 0 }),
            );
            let s = (dv + src).clamp(zero, pix_hi).to_array();
            for j in 0..4 {
                output[idx + j] = s[j] as u16;
...
pub(crate) fn try_inv_txfm2d_rect48_fused(
    ...
    if row_clamp == 16 && col_clamp == 16 && *sr_row == [16i8; 12] && *sr_col == [16i8; 12] {
        ...
        if let (Some((kr, kn_r)), Some((kc, kn_c))) = (pick(txfm_type_row), pick(txfm_type_col)) {
...
pub(crate) fn try_inv_txfm2d_rect48_fused_u8(
    ...
    let (Some((kr, kn_r)), Some((kc, kn_c))) = (pick(txfm_type_row), pick(txfm_type_col)) else {
        return false;
    };
    if kn_r != col_n || kn_c != row_n {
        return false;
...
const INV84_I16_BOUND: [[i32; 3]; 3] =
    [[6204, 3282, 6204], [6426, 3399, 6426], [16390, 8670, 16388]];
...
fn inv_rect816_fused(
    ...
    for rg in 0..rg_n {
        ...
        incant!(
            run_inv1d(kr, &k[..col_n], &mut o[..col_n], cos_bit, sr_row),
            [v3, neon]
        );
        for c in 0..col_n {
    ...
    for cg in 0..cg_n {
        ...
        let mut o = [i32x8::zero(t); 16];
        incant!(
            run_inv1d(kc, &ci[..row_n], &mut o[..row_n], cos_bit, sr_col),
            [v3, neon]
        );
        for r in 0..row_n {
...
#[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))]
const INV8_I16_BOUND: [[i32; 3]; 3] = [[2347, 2431, 6201], [2432, 2518, 6423], [6202, 6423, 16383]];
...
const INV16_I16_BOUND: [[i32; 3]; 3] =
    [[2474, 1238, 6424], [2521, 1261, 3215], [9084, 4546, 11584]];
...
fn inv_w16_fused_i16(
    ...
    let idct16 = |i: &[__m128i; 16]| -> [__m128i; 16] {
        let c = crate::transform::cospi::cospi_arr(cos_bit);
        let mut x = [
            i[0], i[8], i[4], i[12], i[2], i[10], i[6], i[14], i[1], i[9], i[5], i[13], i[3],
            i[11], i[7], i[15],
        ];
        // stage 2
    ...
    let iadst16 = |i: &[__m128i; 16]| -> [__m128i; 16] {
        let c = crate::transform::cospi::cospi_arr(cos_bit);
        let mut x = [
            i[15], i[0], i[13], i[2], i[11], i[4], i[9], i[6], i[7], i[8], i[5], i[10], i[3],
            i[12], i[1], i[14],
        ];
        // stage 2
    ...
    // ---- ymm twins of the 8-pt kernels for the rect shapes ---------------
    // Stacked-halves: each ymm carries the xmm version's two 8-lane groups
    // (row-groups for 8x16's row pass, column-groups for 16x8's col pass).
    // Every op is per-128-half consistent, so the network is bit-identical.
    let pair_y = |a: i32, b: i32| -> __m256i {
        _mm256_set1_epi32((a as u16 as u32 | ((b as u16 as u32) << 16)) as i32)
    };
    let btf_y = |w0: __m256i, w1: __m256i, i0: __m256i, i1: __m256i| -> (__m256i, __m256i) {
    ...
    let adds_subs_y = |a: __m256i, b: __m256i| -> (__m256i, __m256i) {
        (_mm256_adds_epi16(a, b), _mm256_subs_epi16(a, b))
    };
    let subs_adds_y = adds_subs_y;

    // `av1_idct8_sse2` at ymm width.
    let idct8y = |i: &[__m256i; 8]| -> [__m256i; 8] {
        let c = crate::transform::cospi::cospi_arr(cos_bit);
        let mut x = [i[0], i[4], i[2], i[6], i[1], i[5], i[3], i[7]];
        let (x4, x7) = btf_y(pair_y(c[56], -c[8]), pair_y(c[8], c[56]), x[4], x[7]);
        x[4] = x4;
        x[7] = x7;
        let (x5, x6) = btf_y(pair_y(c[24], -c[40]), pair_y(c[40], c[24]), x[5], x[6]);
        x[5] = x5;
        x[6] = x6;
        let (x0, x1) = btf_y(pair_y(c[32], c[32]), pair_y(c[32], -c[32]), x[0], x[1]);
        x[0] = x0;
        x[1] = x1;
        let (x2, x3) = btf_y(pair_y(c[48], -c[16]), pair_y(c[16], c[48]), x[2], x[3]);
        x[2] = x2;
        x[3] = x3;
        let (x4, x5) = adds_subs_y(x[4], x[5]);
        x[4] = x4;
        x[5] = x5;
        let (x7, x6) = subs_adds_y(x[7], x[6]);
        x[7] = x7;
        x[6] = x6;
        let (x0, x3) = adds_subs_y(x[0], x[3]);
        let (x1, x2) = adds_subs_y(x[1], x[2]);
        x[0] = x0;
        x[3] = x3;
        x[1] = x1;
        x[2] = x2;
        let (x5, x6) = btf_y(pair_y(-c[32], c[32]), pair_y(c[32], c[32]), x[5], x[6]);
        x[5] = x5;
        x[6] = x6;
        [
            _mm256_adds_epi16(x[0], x[7]),
            _mm256_adds_epi16(x[1], x[6]),
            _mm256_adds_epi16(x[2], x[5]),
            _mm256_adds_epi16(x[3], x[4]),
            _mm256_subs_epi16(x[3], x[4]),
            _mm256_subs_epi16(x[2], x[5]),
            _mm256_subs_epi16(x[1], x[6]),
            _mm256_subs_epi16(x[0], x[7]),
        ]
    };

    // `av1_iadst8_sse2` at ymm width.
    let iadst8y = |i: &[__m256i; 8]| -> [__m256i; 8] {
        let c = crate::transform::cospi::cospi_arr(cos_bit);
        let mut x = [i[7], i[0], i[5], i[2], i[3], i[4], i[1], i[6]];
        let (x0, x1) = btf_y(pair_y(c[4], c[60]), pair_y(c[60], -c[4]), x[0], x[1]);
        x[0] = x0;
        x[1] = x1;
        let (x2, x3) = btf_y(pair_y(c[20], c[44]), pair_y(c[44], -c[20]), x[2], x[3]);
        x[2] = x2;
        x[3] = x3;
        let (x4, x5) = btf_y(pair_y(c[36], c[28]), pair_y(c[28], -c[36]), x[4], x[5]);
        x[4] = x4;
        x[5] = x5;
        let (x6, x7) = btf_y(pair_y(c[52], c[12]), pair_y(c[12], -c[52]), x[6], x[7]);
        x[6] = x6;
        x[7] = x7;
        for (a, b) in [(0usize, 4usize), (1, 5), (2, 6), (3, 7)] {
            let (s, d) = adds_subs_y(x[a], x[b]);
            x[a] = s;
            x[b] = d;
        }
        let (x4, x5) = btf_y(pair_y(c[16], c[48]), pair_y(c[48], -c[16]), x[4], x[5]);
        x[4] = x4;
        x[5] = x5;
        let (x6, x7) = btf_y(pair_y(-c[48], c[16]), pair_y(c[16], c[48]), x[6], x[7]);
        x[6] = x6;
        x[7] = x7;
        for (a, b) in [(0usize, 2usize), (1, 3), (4, 6), (5, 7)] {
            let (s, d) = adds_subs_y(x[a], x[b]);
        ...
        let (x2, x3) = btf_y(pair_y(c[32], c[32]), pair_y(c[32], -c[32]), x[2], x[3]);
        x[2] = x2;
        x[3] = x3;
        let (x6, x7) = btf_y(pair_y(c[32], c[32]), pair_y(c[32], -c[32]), x[6], x[7]);
        x[6] = x6;
        x[7] = x7;
    ...
    // `iidentity8_sse2` at ymm width.
    let iidtx8y = |i: &[__m256i; 8]| -> [__m256i; 8] {
        [
            _mm256_adds_epi16(i[0], i[0]),
            _mm256_adds_epi16(i[1], i[1]),
            _mm256_adds_epi16(i[2], i[2]),
            _mm256_adds_epi16(i[3], i[3]),
            _mm256_adds_epi16(i[4], i[4]),
            _mm256_adds_epi16(i[5], i[5]),
            _mm256_adds_epi16(i[6], i[6]),
            _mm256_adds_epi16(i[7], i[7]),
        ]
    };

    let run8y = |k: Inv16, i: &[__m256i; 8]| -> [__m256i; 8] {
        match k {
            Inv16::Dct => idct8y(i),
            Inv16::Adst => iadst8y(i),
            Inv16::Idtx => iidtx8y(i),
        }
    };
    ...
    // Rect shapes lane-double their 16-element axis onto ymm (the same
    // stacked-halves construction as `inv_16x16_fused_i16_w16`):
    //   8x16 — the 8-pt ROW pass runs on ymm (`wy[c]` = rows [0-7 | 8-15] of
    //          column c); the 16-pt col pass stays xmm (8 col lanes).
    //   16x8 — the 16-pt row pass stays xmm; the 8-pt COL pass runs on ymm
    //          (`cy[r]` = cols [0-7 | 8-15] of row r).
    if col_n == 8 && row_n == 16 {
        // ymm row pass over the two stacked row-groups.
        let mut wy = [_mm256_setzero_si256(); 8];
        for c in 0..8 {
            wy[c] = _mm256_set_m128i(w[1][c], w[0][c]);
        }
        let o = run8y(kr, &wy);
        let rs = _mm256_set1_epi16(1 << 14);
        for (v, o) in wy.iter_mut().zip(o.iter()) {
            *v = _mm256_mulhrs_epi16(*o, rs);
        }
        // Transpose 8 ymm -> 16 xmm: each ymm's halves are row-groups 0/1;
        // tr8 on the lo halves gives rows 0-7, hi halves rows 8-15.
        let mut m0 = [_mm_setzero_si128(); 8];
        let mut m1 = [_mm_setzero_si128(); 8];
        for c in 0..8 {
            m0[c] = _mm256_castsi256_si128(wy[c]);
            m1[c] = _mm256_extracti128_si256::<1>(wy[c]);
        }
        let t0 = tr8(&m0);
        let t1 = tr8(&m1);
        ...
        if lr_flip {
            for v in ci.iter_mut() {
                *v = _mm_shuffle_epi32::<0x4E>(_mm_shufflehi_epi16::<0x1B>(_mm_shufflelo_epi16::<
                    0x1B,
                >(*v)));
            }
        }
        let o = run(kc, &ci[..16]);
        let mut co = [_mm_setzero_si128(); 16];
        for (v, o) in co.iter_mut().zip(o.iter()) {
            *v = _mm_mulhrs_epi16(*o, _mm_set1_epi16(2048));
        }
        // Write 16 rows x 8 cols (single column-group).
        let zero = _mm_setzero_si128();
        return match output {
            InvDst::U16(output) => {
                let hi = _mm_set1_epi16(((1i32 << bd) - 1) as i16);
                for r in 0..16usize {
                    ...
                    let d = _mm_loadu_si128(dst);
                    let sum = _mm_min_epi16(_mm_max_epi16(_mm_add_epi16(d, co[src_i]), zero), hi);
                    _mm_storeu_si128(dst, sum);
                }
                true
            }
            InvDst::U8(output) => {
                for r in 0..16usize {
                    ...
                    let d = _mm_cvtepu8_epi16(_mm_loadu_si64(dst));
                    let sum = _mm_packus_epi16(_mm_add_epi16(d, co[src_i]), zero);
                    _mm_storeu_si64(dst, sum);
                }
                true
            }
    ...
    if col_n == 16 && row_n == 8 {
        // xmm row pass (16-pt over the 16 column registers).
        let o = run(kr, &w[0][..16]);
        for (v, o) in w[0].iter_mut().zip(o.iter()) {
            *v = _mm_mulhrs_epi16(*o, row_shift);
        }
        // Transpose 16 xmm -> 8 ymm: tr8 on each 8-column half, then stack.
        let ta = tr8(<&[__m128i; 8]>::try_from(&w[0][..8]).unwrap());
        let tb = tr8(<&[__m128i; 8]>::try_from(&w[0][8..]).unwrap());
        let mut cy = [_mm256_setzero_si256(); 8];
        for r in 0..8 {
            cy[r] = _mm256_set_m128i(tb[r], ta[r]);
        }
        // lr_flip reverses all 16 column lanes: half-swap + per-half reverse.
        if lr_flip {
        ...
        // ymm col pass (8-pt over the 8 row registers).
        let oy = run8y(kc, &cy);
        let mut co = [_mm256_setzero_si256(); 8];
        let cs = _mm256_set1_epi16(2048);
        for (v, o) in co.iter_mut().zip(oy.iter()) {
            *v = _mm256_mulhrs_epi16(*o, cs);
        }
        // Write 8 rows x 16 cols.
        let zeroy = _mm256_setzero_si256();
        return match output {
            InvDst::U16(output) => {
                let hi = _mm256_set1_epi16(((1i32 << bd) - 1) as i16);
                for r in 0..8usize {
                    let src_i = if ud_flip { 7 - r } else { r };
                    let idx = r * stride;
                    ...
                    let sum = _mm256_min_epi16(
                        _mm256_max_epi16(_mm256_add_epi16(d, co[src_i]), zeroy),
                        hi,
                    );
            ...
            InvDst::U8(output) => {
                for r in 0..8usize {
                    ...
                    let p = _mm256_permute4x64_epi64::<0x08>(_mm256_packus_epi16(sum, zeroy));
...
fn inv_16x16_fused_i16_w16(
    ...
    for (c, v) in w.iter_mut().enumerate() {
        ...
        mx = _mm256_max_epu32(
            mx,
            _mm256_max_epu32(_mm256_abs_epi32(va), _mm256_abs_epi32(vb)),
        );
    ...
    match output {
        InvDst::U16(output) => {
            ...
            for r in 0..16usize {
                ...
                let d = _mm256_loadu_si256(dst);
                let sum =
                    _mm256_min_epi16(_mm256_max_epi16(_mm256_add_epi16(d, co[src_i]), zero), hi);
                _mm256_storeu_si256(dst, sum);
            }
        }
        InvDst::U8(output) => {
...
pub(crate) fn try_inv_row_pass(
    ...
    if row_clamp == 16 && stage_range.iter().all(|&b| b == 16) && row_n.is_multiple_of(16) {
        if let Some(k16) = lowbd16::inv_kernel_i16(txfm_type_row) {
            ...
            if incant!(
                lowbd16::inv_row_pass_i16(k16, mod_input, buf, col_n, row_n, rect1, shift0_bit),
                [v3, neon, scalar]
            ) {
                return true;
            }
    ...
    while rg < row_n {
        ...
        for (c, ti) in tin[..col_n].iter_mut().enumerate() {
            let mut v = if active == 8 {
                ...
                let a: [i32; 4] = mod_input[c * row_n + rg..c * row_n + rg + 4]
                    .try_into()
                    .unwrap();
...
fn fwd_rect816_fused(
    ...
    for cg in 0..cg_n {
        ...
        let mut o = [i32x8::zero(t); 16];
        incant!(
            run_fwd1d(kc, &v[..row_n], &mut o[..row_n], cos_bit_col, &sr),
            [v3, neon]
        );
        for r in 0..row_n {
    ...
    for rg in 0..rg_n {
        ...
        incant!(
            run_fwd1d(kr, &ri[..col_n], &mut u[..col_n], cos_bit_row, &sr),
            [v3, neon]
        );
...
fn fwd_rect48_fused(
    ...
    let mut w = [i32x8::zero(t); 8];
    incant!(
        run_fwd1d(kc, &v[..row_n], &mut w[..row_n], cos_bit_col, &sr),
        [v3, neon]
    );
...
fn inv_4x4_fused_u8(
    ...
    use archmage::intrinsics::x86_64::*;
    let Some(u) = incant!(inv_4x4_i16_core(kr, kc, input, ud_flip, lr_flip), [v3]) else {
        return false;
    };
    for r in 0..4usize {
        let src = u[if ud_flip { 3 - r } else { r }];
...
fn inv_4x4_i16_core(
    ...
    let iadst4 = |i: &[__m128i; 4]| -> [__m128i; 4] {
        ...
        let srai = |v: __m128i| {
            _mm_packs_epi32(
                _mm_sra_epi32(_mm_add_epi32(v, rnd), cnt),
                _mm_setzero_si128(),
            )
        };
        [
...
fn fwd_col_pass_core(
    ...
    while cg < col_n {
        ...
        incant!(
            run_fwd1d(kernel, &tin[..row_n], &mut tout[..row_n], cos_bit_col, &sr),
            [v3, neon]
        );
        for (r, to) in tout[..row_n].iter_mut().enumerate() {
            let v = if shift1_bit > 0 {
                rshiftv(t, *to, shift1_bit)
            } else {
                *to
            };
...
fn fwd_row_pass_core(
    ...
    for rg in (0..row_n).step_by(8) {
        ...
        incant!(
            run_fwd1d(kernel, &tin[..col_n], &mut tout[..col_n], cos_bit_row, &sr),
            [v3, neon]
        );
...
mod tests {
    ...
    fn cases() -> Vec<Case> {
        ...
        vec![
            Case {
                name: "idct4",
                size: 4,
                scalar: crate::transform::av1_idct4,
                simd: I(Inv1d::Dct4),
            },
            Case {
                name: "idct8",
                size: 8,
                scalar: crate::transform::av1_idct8,
                simd: I(Inv1d::Dct8),
            },
            Case {
                name: "idct16",
                size: 16,
                scalar: crate::transform::av1_idct16,
                simd: I(Inv1d::Dct16),
            },
            Case {
                name: "idct32",
                size: 32,
                scalar: crate::transform::av1_idct32,
                simd: I(Inv1d::Dct32),
            },
            Case {
                name: "idct64",
                size: 64,
                scalar: crate::transform::av1_idct64,
                simd: I(Inv1d::Dct64),
            },
            Case {
                name: "iadst4",
                size: 4,
                scalar: crate::transform::av1_iadst4,
                simd: I(Inv1d::Adst4),
            },
            Case {
                name: "iadst8",
                size: 8,
                scalar: crate::transform::av1_iadst8,
                simd: I(Inv1d::Adst8),
            },
            Case {
                name: "iadst16",
                size: 16,
                scalar: crate::transform::av1_iadst16,
                simd: I(Inv1d::Adst16),
            },
            Case {
                name: "iidentity4",
                size: 4,
                scalar: crate::transform::av1_iidentity4,
                simd: I(Inv1d::Idtx4),
            },
            Case {
                name: "iidentity8",
                size: 8,
                scalar: crate::transform::av1_iidentity8,
                simd: I(Inv1d::Idtx8),
            },
            Case {
                name: "iidentity16",
            ...
            Case {
                name: "fdct4",
                size: 4,
                scalar: crate::transform::av1_fdct4,
                simd: F(Fwd1d::Dct4),
            },
            Case {
                name: "fdct8",
                size: 8,
                scalar: crate::transform::av1_fdct8,
                simd: F(Fwd1d::Dct8),
            },
            Case {
                name: "fdct16",
                size: 16,
                scalar: crate::transform::av1_fdct16,
                simd: F(Fwd1d::Dct16),
            },
            Case {
                name: "fdct32",
                size: 32,
                scalar: crate::transform::av1_fdct32,
                simd: F(Fwd1d::Dct32),
            },
            Case {
                name: "fdct64",
                size: 64,
                scalar: crate::transform::av1_fdct64,
                simd: F(Fwd1d::Dct64),
            },
            Case {
                name: "fadst4",
                size: 4,
                scalar: crate::transform::av1_fadst4,
                simd: F(Fwd1d::Adst4),
            },
            Case {
                name: "fadst8",
                size: 8,
                scalar: crate::transform::av1_fadst8,
                simd: F(Fwd1d::Adst8),
            },
            Case {
                name: "fadst16",
                size: 16,
                scalar: crate::transform::av1_fadst16,
                simd: F(Fwd1d::Adst16),
            },
            Case {
                name: "fidentity4",
                size: 4,
                scalar: crate::transform::av1_fidentity4,
                simd: F(Fwd1d::Idtx4),
            },
            Case {
                name: "fidentity8",
                size: 8,
                scalar: crate::transform::av1_fidentity8,
                simd: F(Fwd1d::Idtx8),
            },
            Case {
                name: "fidentity16",
    ...
    fn assert_batch(
        ...
        incant!(
            run_kernel(case.simd, &vin, &mut vout, cos_bit, stage_range),
            [v3, neon]
        );
