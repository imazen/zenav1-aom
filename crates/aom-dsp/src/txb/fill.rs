...
/// [`crate::txb::write_coeffs_txb`] reads/adapts, e.g. `KfFrameContext::coeff`) —
...
pub fn fill_lv_map_coeff_cost_from_arena(
    ...
    use crate::txb::write::{A_BASE, A_BASE_EOB, A_BR, A_DC_SIGN, A_EOB_EXTRA, A_TXB_SKIP};
...
/// [`crate::txb::write::EOB_OFF`] in a live coefficient-CDF arena -- the SAME
...
pub struct CoeffCostSet {
    /// `coeff_costs[txs_ctx][plane]`, `txs_ctx` = [`crate::txb::txsize_entropy_ctx`]`(tx_size)`.
...
impl CoeffCostSet {
    ...
    /// `tx_size` -- the composed view [`crate::txb::cost_coeffs_txb`] /
    /// [`crate::txb::write_coeffs_txb`] read, selecting the `txs_ctx` coeff
    /// tables and the `eob_multi_size` eob table independently (see the
    /// struct docs).
    pub fn tables(&self, tx_size: usize) -> CoeffCostTables<'_> {
        let txs_ctx = crate::txb::txsize_entropy_ctx(tx_size);
        let eob_multi_size = crate::txb::write::TXSIZE_LOG2_MINUS4[tx_size] as usize;
