...
//!
//! Same aom-rs SIMD pattern as `aom_cdef` / `aom_loopfilter` / `aom_txb`: ONE
...
//! per-row fill) are memset/memcpy slice ops in [`crate::predict_highbd`] — the
...
use crate::weights::SMOOTH_WEIGHT_LOG2_SCALE;
    ...
    for r in 0..bh {
        for c in 0..bw {
            ...
            let p = wh * above_row[c] as i32
                ...
                + ww * left[r] as i32
                + (SCALE - ww) * right;
            dst[r * stride + c] = crate::divide_round(p, log2) as u16;
        }
    }
}
    ...
    for r in 0..bh {
        ...
        while c < bw {
            ...
            let p = wh * above_row[c] as i32
                ...
                + ww * left_r
                + (SCALE - ww) * right;
            dst[row + c] = crate::divide_round(p, 1 + SMOOTH_WEIGHT_LOG2_SCALE) as u16;
            c += 1;
        }
    }
}
...
fn smooth_v_impl(
    ...
    for r in 0..bh {
        ...
        while c < bw {
            let p = w * above_row[c] as i32 + (SCALE - w) * below;
            dst[row + c] = crate::divide_round(p, SMOOTH_WEIGHT_LOG2_SCALE) as u16;
            c += 1;
        }
    }
}
...
pub(crate) fn paeth_scalar(
    ...
    for r in 0..bh {
        for c in 0..bw {
            dst[r * stride + c] =
                crate::paeth_single_i32(left[r] as i32, above_row[c] as i32, top_left) as u16;
        }
    }
}
...
fn paeth_impl(
    ...
    for r in 0..bh {
        ...
        while c < bw {
            dst[row + c] = crate::paeth_single_i32(left_r, above_row[c] as i32, top_left) as u16;
            c += 1;
        }
    }
}
