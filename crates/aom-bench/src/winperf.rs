...
//! * **the bootstrap** is a committed fixture ([`BOOTSTRAP_HEX`]) — the bytes a
//!   real `aomenc --allintra` emitted for this exact cell on the dev box, so
//!   the frame header the port parses is a genuine one.
//!
...
/// The sequence-header bootstrap for [`CELL`], as hex.
///
...
/// produced for [`synth_i420`]`(1024, 1024)` on the dev box. The port reads the
/// sequence header and the uncompressed frame header out of it and produces
/// every coded byte itself — the same arrangement `drv-aom` documents, with the
/// one difference that the encode which produced these bytes happened once, at
...
pub const BOOTSTRAP_HEX: &str = include_str!("../fixtures/winperf_bootstrap_1024x1024_cq44_s6.hex");

/// [`BOOTSTRAP_HEX`] decoded. Panics on any non-hex byte — the fixture is
/// checked in, so a failure here is a corrupted tree, not a runtime condition.
pub fn bootstrap() -> Vec<u8> {
    let mut out = Vec::with_capacity(BOOTSTRAP_HEX.len() / 2);
    let mut hi: Option<u8> = None;
    for c in BOOTSTRAP_HEX.bytes() {
        let v = match c {
            b'0'..=b'9' => c - b'0',
            b'a'..=b'f' => c - b'a' + 10,
            b'A'..=b'F' => c - b'A' + 10,
...
pub fn synth_i420(w: usize, h: usize) -> Vec<u8> {
    ...
    for y in 0..h {
        for x in 0..w {
            // fBm: period 256..4, amplitudes chosen so the coded size and the
            // allocator census land near the dev box's 1 MP photograph rather
            // than near a smooth gradient (see the module docs: the point is a
            // broadly photographic spectrum, and a steep 1/2-per-octave ladder
            // is markedly SMOOTHER than real photos — it coded 2316 bytes at
            // this cell against the photo's 4472).
            let mut acc = 0i32;
            ...
            out[y * w + x] = (noise + ramp - 20).clamp(0, 255) as u8;
        }
    }
    for y in 0..ch {
        for x in 0..cw {
...
pub fn cell(w: usize, h: usize, cq: i32, speed: i32) -> EncodeCell {
    let buf = synth_i420(w, h);
    let (cw, ch) = (w / 2, h / 2);
    let up = |s: &[u8]| s.iter().map(|&b| u16::from(b)).collect::<Vec<u16>>();
    EncodeCell {
        label: "winperf".to_string(),
        w,
        h,
        mono: false,
        ss_x: 1,
...
mod tests {
    ...
    fn synth_source_is_pinned_and_not_blank() {
        ...
        assert_eq!(
            (y, u, v),
            (159_114_483, 34_350_299, 33_351_188),
            "winperf synthetic source changed — every recorded winperf timing \
             was taken against the old one and is no longer comparable"
        );
        // Not blank, and not saturated.
        ...
        let var = buf[..w * h]
            ...
            .sum::<i64>()
            / (w * h) as i64;
        assert!(var > 400, "luma variance {var} too low to be photographic");
        // Observed luma range at this cell is 66..=232; assert a band strictly
        // inside it, so a flat or low-contrast plane fails outright.
        assert!(buf[..w * h].iter().any(|&b| b < 90));
        assert!(buf[..w * h].iter().any(|&b| b > 200));
    }

    /// The committed bootstrap is a real section-5 stream for the study cell.
    #[test]
    fn bootstrap_fixture_decodes_and_is_a_stream() {
        let b = bootstrap();
        assert!(b.len() > 64, "bootstrap fixture is {} bytes", b.len());
        // First OBU must be a temporal delimiter (type 2) with obu_has_size.
        assert_eq!((b[0] >> 3) & 0xf, 2, "first OBU is not a temporal delimiter");
        assert_eq!((b[0] >> 1) & 1, 1, "obu_has_size_field not set");
    }
}
