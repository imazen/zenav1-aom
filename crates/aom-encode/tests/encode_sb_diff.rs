...
use aom_encode::intra_uv_rd::{av1_get_tx_size_uv, chroma_plane_offset};
...
struct COracle<'a> {
    ...
    dequant_u: [i16; 2],
    dequant_v: [i16; 2],
    coeff_tbls_y: CCoeffTbls<'a>,
    coeff_tbls_uv: CCoeffTbls<'a>,
    ttc: (&'a [i32], &'a [i32]),
...
type CLeafOut = (i32, i32, usize, bool, bool, Vec<CTxb>, Option<Vec<CTxb>>, Option<Vec<CTxb>>);
...
impl COracle<'_> {
    ...
    fn encode_b(
        ...
        let ref_off_y = self.base_y + (mi_row as usize * 4) * STRIDE + mi_col as usize * 4;
        ...
        let ca = CEncPlaneArgs {
            bsize,
            tx_size: w.tx_size,
            geometry: (mi_row, mi_col, ref_off_y, ref_off_y, STRIDE),
            sb_size: 12,
            src: self.src_y,
            mode: w.mode,
        ...
        if !self.monochrome && chroma_ref {
            let ref_off_uv =
                chroma_plane_offset(self.base_uv, STRIDE, mi_row, mi_col, bsize, ss_x, ss_y);
            let plane_bsize = aom_entropy::partition::get_plane_block_size(bsize, ss_x, ss_y);
            let (pmw, pmh) = (MI_W_B[plane_bsize], MI_H_B[plane_bsize]);
    ...
    fn encode_sb(
        ...
        let hbs = (MI_W_B[bsize] / 2) as i32;
        let (partition, subsize) = match tree {
            SbTree::Leaf(_) => (0i32, bsize),
            SbTree::Split(_) => (3i32, split_subsize(bsize)),
        };
        match tree {
            SbTree::Leaf(w) => {
