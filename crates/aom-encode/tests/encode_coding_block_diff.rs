...
use aom_encode::{
    encode_coding_block_plane, BlockContexts, OptimizeInputs, QuantKind, QuantParams, TxTypeContext,
};
use aom_entropy::enc::OdEcEnc;
use aom_sys_ref as c;
use aom_txb::{ext_tx_derive, scan, write_coeffs_txb_full, CoeffCostTables, CDF_ARENA_LEN};
...
fn encode_coding_block_plane_identical() {
        ...
        for &plane in &[0usize, 1] {
            for iter in 0..24 {
                ...
                let residuals: Vec<Vec<i16>> = (0..n_txb)
                    .map(|_| (0..full).map(|_| rng.range(-(rmax - 1), rmax) as i16).collect())
                    .collect();
                ...
                let kind = match iter % 3 { 0 => QuantKind::Fp, 1 => QuantKind::B, _ => QuantKind::Dc };

                let cost_v = |rng: &mut Rng, n: usize| -> Vec<i32> { (0..n).map(|_| rng.cost()).collect() };
                ...
                let ttx = TxTypeContext {
                    is_inter: iter % 2 == 0, reduced: iter % 5 == 0, use_filter_intra: false,
                    fi_mode: 0, mode: [0usize, 1, 2, 6, 12][iter % 5], signal_gate: true,
                };
                // The whole block shares one tx_type/context, so one ext-tx set num.
                let d = ext_tx_derive(tx_size, ttx.is_inter, ttx.reduced, tx_type, false, 0, ttx.mode);
                ...
                let bc_r = encode_coding_block_plane(
                    &res_refs, plane_bsize, tx_size, tx_type, kind, &qp, &opt, &ttx, plane, upd,
                    &mut enc_r, &mut arena_r, &mut ext_r,
                );
                ...
                while blk_row < uh {
                    ...
                    while blk_col < uw {
                        let (skip_ctx, dc_sign_ctx) =
                            c::ref_get_txb_ctx(plane_bsize, tx_size, plane, &above[blk_col..], &left[blk_row..]);
                        let coeff_c = c::ref_fwd_txfm2d(tx_size, &residuals[idx], TX_W[tx_size], tx_type);
                        ...
                        write_coeffs_txb_full(
                            &mut enc_c, &mut arena_c, &mut ext_c, &qc, eob_w, tx_size, tx_type,
                            plane_type, skip_ctx as usize, dc_sign_ctx as usize, upd, ttx.is_inter,
                            ttx.reduced, ttx.use_filter_intra, ttx.fi_mode, ttx.mode, ttx.signal_gate,
                        );
                ...
                let m = format!("pb={plane_bsize} ts={tx_size} pl={plane} kind={kind:?} bd={bd} n_txb={n_txb}");
    ...
    assert!(multi_txb * 2 >= total, "too few multi-txb blocks: {multi_txb}/{total}");
}
