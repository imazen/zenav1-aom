...
// lowbd_lpf_sse2_shim.c — the REAL dispatched x86-64 lowbd kernels
// (aom_lpf_{horizontal,vertical}_{4,6,8,14}_sse2), not the `_c` references.
// These differ from `aom_lpf_*_c` exactly where C's own kernels differ: the
// mask sums use saturating u8 arithmetic, so when blimit+limit >= 255 a
// capped sum can pass the criterion where the unclamped scalar sum fails it.
#[cfg(target_arch = "x86_64")]
extern "C" {
    fn shim_lowbd_lpf_sse2(dir: i32, width: i32, s: *mut u8, p: i32, bl: u8, li: u8, th: u8);
}

/// Apply a reference lowbd loop filter via the real dispatched SSE2 kernels.
/// `dir`: 'h'/'v'. x86-64 only.
#[cfg(target_arch = "x86_64")]
#[allow(clippy::too_many_arguments)]
pub fn ref_lpf_sse2(
