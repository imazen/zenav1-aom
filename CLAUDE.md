...
2. **Refusals are not divergences and must close.** A configuration a caller can reach must
   ...
   documented refusals in both directions). The SCM trial resolved 2026-09-13 as **KB-66: unreachable
   in this envelope** (one-pass + no lookahead forces `DISALLOW_RECODE`; C never runs it), not a
   refusal and not a divergence — and landed the same day as the first `KeyFrameMode::Zenaom`
   opt-in (KB-67; `LibaomExact` stays the default and is byte-unchanged); the harness
   `aom-bench` may still refuse cells it cannot model.
...
| clause | state |
|---|---|
| (1) a backend zenavif can select **by default** | Seam **works**, re-verified live at HEAD 2026-09-10 against this repo (not the pinned rev) via a temporary `[patch]` compose: `aom_encode_backend` 22/22, `aom_roundtrip_loss` 5/5, `resolved_routing` 6/6. "By default" needs two one-line zenavif flips (`default` feature set; `#[default]` on `Av1Backend`) and is a **decision**, not a measurement: against the BAR the port is 1.94x libaom (row 4); against the INCUMBENT zenavif default (`Zenravif`, a rav1e fork, speed 4 = our `--cpu-used 3`) it is **10.2x faster, 0.5 % fewer bytes, +3.0 SSIMULACRA2**, and threading rav1e buys 3 % (`benchmarks/encoder_vs_default_backend_2026-09-10.md`). `screen_likelihood` is the zenanalyze `patch_fraction`
   seam (hint only; no dep from `aom-encode`). **2026-09-13: every `pub` type on the
   `key_frame` surface is now `#[non_exhaustive]`** (structs AND enums — snapshot pins it) and
   **the 2026-09-16 pin bump (`fbea6b4` -> `41c9c065`, zenavif branch `bump/zenav1-aom-encode-41c9c065`)
   DID the `allintra_speed0(..)` + field-mutation migration** — field additions stop being
   two-repo breaks. That bump's new `validate_configuration` also caught a seam bug: the
   alpha aux item's mono encode inherited MC_IDENTITY, which upstream refuses on mono. Re-run the `[patch]` compose whenever a `pub` signature in `aom-dsp`/`aom-encode` changes. Full history: `docs/CLAUSE_STATUS_LOG.md`. |
...
| (4) encode time within libaom | **MET at the user's ≤1.40× ask — median 1.384× at 1024x1024 cq27 `--cpu-used 3`, 2026-09-15, branch `perf/gate3-txfm-i16-batch`** (port ~2207 ms / C ~1597 ms, 10 interleaved pairs, both arms byte-identical 40,237 B; 9/10 pairs < 1.40 — margin ~1% vs ±1.5% run noise, so treat as met-but-thin). Landings that took 1.94x → 1.38x this cycle: HOG per-SB gradient cache (C `pixel_gradient_info`, −470M Ir at 512²), z2_left band-major tile transpose, filter_intra_edge interior chunking, txb_init_levels/nz_map memset elimination, transform-dispatch vector guards, `optimize_txb` tx_class monomorphization (C's `UPDATE_COEFF_*_CASE`). Named residual gaps, measured not closed: the **u16-at-bd8 tax** (C runs `av1_lowbd_*` u8 kernels at bd8 — restoration ~800M vs ~380M and `highbd_variance64` ~113M vs ~16M at 196² s0; the port's u8 path exists but the encoder never routes to it — a structural program) and safe-Rust bounds-check/min-max overhead in the trellis under `forbid(unsafe_code)`. Record: `benchmarks/encoder_ship_cell_1_40_2026-09-15.md`. Prior readings on the same cell: 1.943x (2026-09-10, port 3011.65 / C 1548.93, 8 rotated rounds), then ~1.55x post-banding (`7099a04` `OnceLock`-cached every hot-path `AOM_*` env read — getenv was 1.40 % flat — port 2476.47 / C 1595.60, 8 reps; the SCM trial `be73e3a` is `Zenaom`-only and costs the default path nothing). The IntraBC screen-content cliff documented in KB-41 (~80 s/1080p) was stale — `b3275d7` measured and cut it to ~0.75 s, ~2.5x C, byte-identical. The 2026-09-08..10 cycle took it 3.24x -> 1.94x over ~45 byte-identical landings (transform fusion, lane-width kernels, bounds-check removal, allocation cuts) and ~15 measured rejections. Two things decide what to build next, both measured: closing EVERY named lever completely is only **27 % of the 716 ms the 1.5x bar needs** (`benchmarks/encoder_lever_map_s3_2026-09-10.md`), so the route is BREADTH — halving every class gap, roughly eight more transform-class-sized cycles; and tile threading is a **~3.2x at 4 threads / ~5.1x at 8** ABSOLUTE-time lever that does NOT move the ratio (`benchmarks/encoder_tile_threading_ceiling_2026-09-10.md`). **2026-09-16: threading landed and the threaded-vs-threaded bar is met — `e7cdd08`, measured against a real multithread libaom v3.14.1 `aomenc` (the byte oracle is `CONFIG_MULTITHREAD=0`): 4t port 666.5 ms / C 607.6 ms = 1.097x and 8t port 435.9 / C 441.5 = 0.988x on like-for-like row-tile geometry; 1.055x vs C's best (4x4 tiles) at 8t. Within the user's <=1.10x at both thread counts. `74fa859` fronts all spawn sites with `aom_dsp::par` (opt-in `rayon` feature for shared-pool hosts; `threads=0` = auto); the full 1-16t curve and content matrix live in STATUS.md — port scales better than C's tile-mt at every point, screen content (IntraBC) is the one class still outside the bar (1.70x@4t).** The port makes ~4x libaom's allocator calls in ~0.75x its footprint, and allocation levers are worth ~6.9x more on Windows than on glibc. **2026-09-16: tile threading LANDED (`6ada3da`) — `KeyFrameConfig::threads` (default 1), phase-1 tile-row bands over `split_at_mut`, no `unsafe`, byte-identical to serial; ~2.76x @4 / ~4.4x @8 threads at 1024² s3, flat heap** — and the bd8 lpf tax partially closed (`41c9c06`: u8 `filter_block_plane_u8_opt` via a narrow-run-widen workspace; restoration + `highbd_variance` remain the u8 structural program). **Rules for choosing a lever, with their evidence, are in `docs/ITERATION_PLAYBOOK.md`; the COMPLETE measured record of this row (~120 KB: every landing, null, rejection and self-correction) is `docs/CLAUSE_STATUS_LOG.md` row (4). Read it before re-deriving anything, and append there.** |
| (5) match the RD of C | byte identity is the strongest available evidence and holds on 549/549 standalone cells — including the entire former `HBD_OPEN` band (KB-61) — and on every speed-0 pinned band: `MONO_S0_OPEN` (KB-27), `SPEED0_1080P_OPEN`/`HD_HBD_OPEN` (KB-38's residual) and the mono content-near-ties all closed 2026-09-13 as KB-62 (the AB-reuse clone's stale `tx_type_map`; C aliases `xd->tx_type_map` to the ctx array so the copied map is post-dry-run); the size-axis finding-B cells closed the same day as KB-63 (`winner_tx_type_map` consumed chunk-ordered winners in raster order — a permuted committed `tx_type_map` on >64px leaves, SB128 only). Still-open pinned divergences, each measured/attributed/bounded: the cpu-8 photo rows needing content not in-repo, and the re-attributed "14 tiny" fleet class (KB-66 struck its C3/SCM-trial attribution — the trial is unreachable in this envelope; needs a new root when the fleet planes are re-staged). (`NONRD_CQ63_OPEN` also re-measured clean — KB-58's per-SB-qindex plumbing had closed it; the pin sat stale since `ae93253`.) |
| (6) sensible conversion + wiring + testing of all of the C encoder | **2026-09-11: every harness-only feature is now REACHABLE from `encode_key_frame`** — `quality` (tune IQ/SSIMULACRA2 bundle via `apply_tune`, QM, sharpness, chroma delta-q, delta-q modes 2/3/6, delta-lf, adaptive CDEF), `tools` (the 24 C8–C11 toggles), `film_grain`, `superres_denom` — each byte-gated against a MATCHED oracle (`ref_encode_av1_kf_cfg`) with a decode round-trip on every cell (`self_contained_tools.rs`, 236 cells, all conformant). Byte parity NOW: superres **18/18**, film grain **10/10**, coding tools **48/48** (KB-60, 2026-09-13: under `--intra-dct-only` C searches chroma tx as DCT but `dist_block_px_domain`/inverse re-derive the UV-mode type via `av1_get_tx_type` — decode-consistent, so the port mirrors it), quality knobs alone **75/75**, the tune bundle **84/84 at s0/s3 + 8/8 at s6/s8** (KB-59, 2026-09-13: `x->rdmult` is per-NODE under a perceptual tune and C's three sub-block call sites fold it three different ways — the port now mirrors each scope). `cdf_update_mode = 0` is REFUSED by name: its stream is rejected by the real decoder (KB-53). Palette + IntraBC wired 2026-09-10 (54/54, matched oracle). The SCM trial encode was ported, verified byte-identical to C's `AOM_RC_LAST_PASS` stream on a flipping cell, found **unreachable-in-envelope** (KB-66), and **re-landed as the first `KeyFrameMode::Zenaom` opt-in** (KB-67: margin-gated on the detector's net score / a `screen_likelihood` hint; 1.07x-1.19x encode cost only on nominated frames, 1.00x on photos; `LibaomExact` default unchanged and byte-exact). Full history: `docs/CLAUSE_STATUS_LOG.md`. |

**The one-line reading:** all six clauses are now either met (incl. encode time at the
user's ≤1.40× ask — 1.384× median, thin margin) or reduced to measured/attributed/bounded
residuals. Perf work still ranks first to widen the 1.40 margin; the named residual is the
u16-at-bd8 structural tax.
...
- **KB-70** — aarch64 CI: the first ARM run of the merged branch was red on 10 differentials — 3 x86-only contracts gated to x86-64, 3 UB-domain harness draws made x86-only (KB-ARM-FLOAT precedent), the lowbd fuzz oracle made pin-aware — FIXED ✅ 2026-09-25; the 5 encode-level `_c`-chain unit tests assert on the forced-scalar ARM leg only (whole-frame ARM byte gates pass) — OPEN for NEON-chain oracle shims, needs ARM hardware
- **KB-69** — Encoder: peak memory grew ~4x over main (222 vs 54.5 MB at 1024² s0) from `e1a97fe`'s per-leaf payload retention, and the KB-50 estimate under-stated it up to 3.2x — BOTH FIXED ✅ 2026-09-24: `RetainedLeaf` keeps only what the pack reads (12-B txb headers + one `i16`/`i32` eob-trimmed level slab per leaf), 222 -> 66.7 MB byte-identical and ~1 % faster, at/below main everywhere except +22 % at full-RD on noise (the retained levels themselves); model re-fitted speed/chroma/thread-aware, 22/22 cells, slack 1.34x..3.21x
- **KB-68** — Encoder: real screen content (`screen_512.yuv`) diverged at an IntraBC leaf — closed by two stacked fixes: phase-2 repack seeded from src instead of phase-1 recon (`2931675`), and `predict_skip_levels` plumbed the MODE_EVAL column into an arm that runs DEFAULT_EVAL (`347a8c8`) — CLOSED ✅ 2026-09-24, witness byte-identical
- **KB-67** — Encoder/mode split: `KeyFrameMode::{LibaomExact, Zenaom}` — SCM trial re-landed as the first opt-in zenaom deviation, margin-gated (detector net score > 0 or `screen_likelihood` hint); decision agreement with C's own two-pass trial, 1.00x photo cost — LANDED 2026-09-13
- **KB-66** — Encoder: `av1_determine_sc_tools_with_encoding` (the C3 "SCM trial") is UNREACHABLE in the port's envelope — its only call site sits in `encode_with_recode_loop`, and one-pass + no lookahead (aomenc allintra forces lag=0 AND passes=1) forces `DISALLOW_RECODE` — ported, verified byte-identical vs C's LAST_PASS stream on a flipping cell, REVERTED as a parity regression; the "14 tiny" fleet class needs re-attribution (its s6/s8 cells can't be the trial under any usage) — RESOLVED 2026-09-13
- **KB-65** — Encoder: coded-lossless IntraBC emitted quadtree-DFS txbs where every decoder reads a flat raster (`get_vartx_max_txsize` = TX_4X4 at lossless collapses C's whole walk) → arith desync → non-conformant streams rejected by the real C decoder with `Invalid intrabc dv`; PLUS the search-time `allow_intrabc` never ANDed `enable_intrabc` (encodeframe.c:2194) → +51/leaf phantom flag in knob-off screen encodes flipping partition near-ties; PLUS the missing `choose_smallest_tx_size` arm — FIXED ✅ 2026-09-13, 47/48 probe cells byte-exact, the 48th a recon-identical fi-vs-PAETH near-tie (documented residual)
- **KB-64** — Encoder: `intra_model_rd` walked model-prediction tiles in mu-64 chunk order where C walks flat raster — observable on any leaf >64px since tiles predict off in-plane predictions; flipped the top-k prune set — FIXED ✅ 2026-09-13, `mono_cq63` SB128 pin promoted to a byte gate
- **KB-63** — Encoder: `winner_tx_type_map` consumed `txfm_rd_in_plane_intra`'s mu-64-chunk-ordered `TxbWinners` in flat raster order, permuting the committed `tx_type_map` on SB128 leaves >64px wide/tall — FIXED ✅ 2026-09-13, closing the size-axis finding-B cells (480x480 p140 / ab0-p140, 512x512 ab0); the "content near-tie" attribution was wrong about mechanism
- **KB-62** — Encoder: the rect-stage AB-reuse clone snapshot `w0` BEFORE the mid-stage dry-run reset its `tx_type_map` — C's `av1_update_state` ALIASES `xd->tx_type_map` to the ctx array, so the ctx the AB stage copies is POST-reset — FIXED ✅ 2026-09-13, closing `MONO_S0_OPEN` (KB-27), `SPEED0_1080P_OPEN`/`HD_HBD_OPEN` (KB-38's residual — the "≥1080p" shape was a near-tie counting artifact, not a predicate boundary), the crop-axis mono pins, and `scr_mono_b10/cq32/diag=0`
...
- **KB-57** — Encoder: `CDEF_ADAPTIVE` qindex thresholds compared the raw 0..=63 cq dial instead of the mapped qindex — FIXED ✅ 2026-09-12
- **KB-56** — Encoder: the `--enable-cdef=1` speed >= 4 header divergence was `sf.cdef_pick_method` never set past LVL1 — FIXED ✅ 2026-09-12
- **KB-55** — Encoder: the `--cpu-used` >= 7 VAR_BASED_PARTITION divergence was the phase-2 repack folding the ALLINTRA per-SB rdmult modifier — FIXED ✅ 2026-09-12
- **KB-54** — Encoder: a `>=1080p` speed-0 divergence class was C's `winner_mode_params` SNAPSHOT, not a live sf write — FIXED ✅ 2026-09-12
- **KB-53** — Encoder: the RD/tune/tool knobs (tune IQ/SSIM2, QM, sharpness, chroma/delta-q, adaptive CDEF, C8–C11 toggles, film grain, superres) were HARNESS-ONLY — WIRED into `encode_key_frame` 2026-09-11 with a matched oracle; superres 18/18 + grain 10/10 byte-exact, tune bundle closed 84/84 by KB-59, `cdf_update_mode=0` refused (corrupt stream)
...
- **KB-27** — Encoder: MONOCHROME at `--cq-level 24` (`base_qindex` 96), speed 0 — a single-point near-tie — CLOSED ✅ 2026-09-13 by KB-62 (was never mono-specific: the AB-reuse stale `tx_type_map`)
...
- **KB-38** — Encoder: `av1_set_speed_features_qindex_dependent`'s `is_1080p_or_larger && base_qindex <= 108` sub-block was UNMODELLED — PORTED ✅ 2026-08-04; the residual pins were KB-62's stale-map near-tie, CLOSED ✅ 2026-09-13 (`SPEED0_1080P_OPEN`/`HD_HBD_OPEN` = `&[]`)
