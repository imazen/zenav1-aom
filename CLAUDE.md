...
* The gate had THREE cq-0 cells before this landing (`A_cq0_64x64_420_bd8_tex`,
  `K2_cq0_59x128_420_bd8_tex_s4`, `M_128x128_cq0_s0_cdef0_lr0_sb128`), all bd8 4:2:0 and
  none of them mono / 4:2:2 / 4:4:4 or bd10 / bd12. Everything else about the coded-lossless
  arm was unmeasured.

**Coverage now** (`benchmarks/cq0_lossless_axis_2026-09-03.md`):

* `self_contained_key_frame_byte_matches_real_aomenc`: **241 -> 427/427 byte-identical**
  (re-measured 2026-09-04 on the merged tree; the arm adds 186 cells and the gate's cq-0
  population is 189).
  The N arm is cq 0 x {mono, 4:2:0, 4:2:2, 4:4:4} x bd {8, 10, 12} x 5 contents x
...
**Mutation proof.** Reverting `count_leaf` to the pre-fix paraphrase turns 4 of the file's 9
tests red (`assertion failed: depth <= MAX_VARTX_DEPTH` at
`aom-dsp/src/entropy/partition.rs:706`, first cell `LL_mono_bd8_flat_64x64_cq0_s0`); forcing
`coded_lossless = false` takes the byte gate to **238/427**, i.e. exactly the 189 cq-0 cells
and nothing else (186 `N_*` + `A_cq0_64x64_420_bd8_tex` + `K2_cq0_59x128_420_bd8_tex_s4` +
`M_128x128_cq0_s0_cdef0_lr0_sb128`). Both reverted, sha256 of both sources restored
byte-identically, 427/427 + 248/248 green again. Re-run in full on the merged tree
...
high-bit-depth encoder-side predictors"). Both aarch64 legs and all three portability legs
are green throughout, so every root is x86-64-only. `gh run view --log-failed` shows only
ONE of them — the other two are process CRASHES with no assertion, so grep the raw job log
(`gh api repos/imazen/zenav1-aom/actions/jobs/<id>/logs | tr -d '\r' | grep -A5 "targets failed"`)
instead of trusting the failed-step summary.

**Root #1 — FIXED 2026-09-02 (`cf98bb3`), verified on run `33688716692`:**
`inter_pred_enc_diff::highbd_comp_mask_upsampled_pred_matches_c`. The ORACLE was wrong, not
the port: `aom_highbd_comp_mask_pred_avx2`'s `width == 8` path
(`upstream/aom_dsp/x86/variance_avx2.c:459`) loads the ODD row's mask from `mask + 8` — a
hardcoded width — while the loop advances by `mask_stride << 1` (`:474`), so at
`mask_stride != width` every odd row blends against the previous row's mask bytes (rows
0/2/4/6 identical, 1/3/5/7 wrong — the exact reported signature). Its SSE2 tier, its NEON
tier and its own 8-bit twin all use `mask + mask_stride`; libaom never reaches the arm
because every production caller and its own test pass `mask_stride == width`. Fixed by
pinning the oracle: `shim/reconinter_enc_shim.c` compiles libaom's `reconinter_enc.c` with
`aom_{,highbd_}upsampled_pred` and `aom_{,highbd_}comp_mask_pred` rebound to their `_c`
tiers (the `shim/cnn_cscalar.c` technique), and `shim/comp_pred_shim.c` routes its two
comp-mask upsampled entries to those copies. Verified at the object level:
...
- **CLOSED 2026-08-30 (KB-41 roots #7-#13, zenav1-aom `38a92657`):** both cells byte-exact on
  replay — `kb13_cpu3_cq63.rs` (self-promoting pin, `KB13_OPEN = &[]`): 128x128@64,64 cq63
  cpu3 = 77 B both sides, 64x64@96,64 cq63 cpu3 = 60 B both sides, neither screen-detected.
  Consistent with root #12 (the search-ctx `tx_size_cdf` shadow — at cq63 no transform splits,
  so the header ends TX_MODE_LARGEST while the search ran at TX_MODE_SELECT); not bite-proved
  by per-root revert. *(Record as of 2026-08-03 follows.)*
...
them a rejected variant).

- **THE DEFECT.** `get_fwd_txfm_cfg` + `get_inv_txfm_cfg` cost **46.7 + 34.5 =
  81.2 ms** at 1 MP and have **NO C counterpart at all** — libaom's whole-2-D
  entry points are size-specialised at compile time, so it never derives a config
  per transform. Pure port overhead, not a kernel gap. Two avoidable parts: the
  `Cfg` struct stored `func_col`/`func_row` (two 12-arm match dispatches) which
  are read **only on the scalar fallback path** the SIMD passes usually skip; and
  `log2_idx` was a five-arm `match` over `{4,8,16,32,64}` called twice per config
  where `trailing_zeros() - 2` is one instruction. Deriving on demand is EXACT:
  `valid` is asserted at all three entry points (`txfm2d.rs:336`,
  `inv_txfm2d.rs:246`/`:396`) before any core runs, so the old
  `else { av1_fdct4 }` arm was already unreachable from the cores.
- **THE FINDING IS THE REJECTED FIRST ATTEMPT, and it is why this entry exists.**
  The obvious edit puts `txfm_func(cfg.txfm_type_col)` at the call site — but in
  BOTH cores the call site is **inside the per-column / per-row loop**, so that
  moves the match from once-per-transform to once-per-column. Measured on the
  same cell, same protocol, 24 rounds:

  | variant | paired | rounds | p |
  |---|---:|---:|---:|
  | lookup at the call site (inside the loop) | **+0.20 %** | 5/24 | 0.0066 |
  | lookup hoisted to the top of each scalar pass | **-0.24 %** | 21/24 | 0.0003 |

  Same two stores and two matches removed per transform; **the only difference is
  which SCOPE the surviving lookup sits in — a 0.44 pp swing, and the wrong
  version was significant in the WRONG direction.** It would have landed as a
  regression by anyone reading the diff instead of the band. The rejected band is
  committed as `.band512_in_loop_rejected.tsv`. **When replacing a stored value
  with a recomputation, check what loop nest the recomputation lands in.**
- **MEASURED** (rotated arms, same-binary null, byte identity verified directly
  on two cells before timing — 39,694 B at 1024x1024, 10,912 B at 512x512):
  **-0.30 % at 1024x1024 (14/16, p=0.0042)** and **-0.24 % at 512x512 (21/24,
  p=0.0003)**; nulls +0.01 % / +0.11 %. Ratio **2.402x -> 2.395x**.
- **Not covered:** the config is still DERIVED per transform. Removing that means
  either a `const` table of 16x19 `Cfg` (~14 KB — trades arithmetic for cache, so
  it must be measured, not assumed) or hoisting the config to the caller, which
  knows `(tx_type, tx_size)` is fixed for a whole walk. Both are larger changes
  and neither is obviously a win.
- **Gates:** `just gate-landing` green in full — `test-next` **1503/1503**,
  `test-next-scalar` **1503/1503**, `census-gate` 4/4, `test-whereat` 4/4.

### KB-PERF-13 — Encoder: three heap allocations per `txfm_rd_in_plane_intra` call, one growing element by element — LANDED ✅ 2026-09-09 (2.410x -> 2.398x at 1 MP, byte-identical)

Record: `benchmarks/encoder_txb_walk_alloc_2026-09-09.md` (+ two band TSVs).

- **FOUND BY THE SYMBOL THAT NAMES A TYPE.** The re-profile taken after
  KB-PERF-9..12 put the **memory class at 559.4 ms**, still the 12x-vs-C outlier.
  Its composition: `__memset` 155.3, `__memmove` 128.9,
  `RawVecInner::finish_grow` 65.0, malloc/free machinery 124.1,
  `do_reserve_and_handle` 19.3 — and **`RawVec<tx_search::TxbWinner>::grow_one`
  9.1**. That last one names a TYPE, which is the whole lead: a `Vec<TxbWinner>`
  pushed one element at a time, reallocating and memcpying, which also explains
  a share of the `finish_grow` and `memmove` above it. **A generic allocator
  symbol is not actionable; a monomorphised one is.**
- **THE DEFECT: three allocations per call** in the per-block transform search
  walk — `t_above`/`t_left` as `to_vec()` copies of the neighbour entropy
  contexts (at most **32 bytes each**, since `MI_SIZE_WIDE_B`/`MI_SIZE_HIGH_B`
  top out at 32 for BLOCK_128X128 in 4x4 units — they never needed a heap), and
  `winners` as `Vec::new()` + per-txb `push`.
- **THE FIX.** `[i8; 32]` arrays with a `debug_assert` on the bound, and
  **every use sliced to `max_blocks_wide`/`max_blocks_high`** so the lengths
  handed to `get_txb_ctx` and to the per-txb `above:`/`left:` fields are exactly
  what the `Vec` form passed — a longer slice would be a silent behaviour change,
  not merely a faster one. `winners` becomes `with_capacity(n_txbs)`.
- **THE COUNT IS KNOWABLE BEFORE THE WALK**, which is what makes an exact
  ...
  `n_txbs = blocks_wide_visible.div_ceil(txw_unit) * blocks_high_visible.div_ceil(txh_unit)`.
  Deliberately NOT `max_blocks_*`-based — that over-allocates to 1024 entries
  (~24 KB) for a TX_4X4 walk on a large block, trading growth for a worse
  allocation. And deliberately NOT moved into `IntraTxScratch` even though it is
  already threaded in: `winners` is RETURNED, so a scratch buffer would mean
  changing the signature and every caller.
- **MEASURED** (rotated arms, same-binary null, byte-identical output verified
  DIRECTLY on two cells before any timing — 39,694 B at 1024x1024 and 10,912 B
  at 512x512):
  **-0.50 % at 1024x1024 (14/14, p=0.0001)** and **-0.53 % at 512x512 (22/24,
  p<0.0001)**; nulls +0.13 % / -0.10 %. Ratio **2.410x -> 2.398x** (C median
  4208.8 ms, same band).
- **THE CALL-COUNT CENSUS WAS NOT TAKEN, and that is a named gap.** For an
  allocation fix the calls removed are stronger evidence than a timing delta, and
  the instrument exists (`examples/eprof_alloc.rs`, used by KB-PERF-1 and -2) —
  but it takes a `.yuv` PATH and this cell is the in-repo mirror-tiled
  `av1-1-b8-01-size-196x196`, which the harness synthesises. **Wiring
  `eprof_alloc` to `EncodeCell` would make every future allocation lever cheaper
  to prove** and is the follow-up.
- **Not covered:** `__memset` 155.3 ms and `__memmove` 128.9 ms are still the
  BULK of the memory class and are untouched — KB-PERF-2's lesson applies
  directly (*"most of the mass of `alloc/libc` is memset/memcpy, not allocator
  bookkeeping — split them before crediting any lever with the stage total"*);
  this landing addressed the bookkeeping half. Other `to_vec()`/`Vec::new()`
  sites on hot paths were not swept.
- **Gates:** `just gate-landing` green in full — `test-next` **1503/1503**,
  `test-next-scalar` **1503/1503**, `census-gate` 4/4, `test-whereat` 4/4.

### KB-PERF-12 — Encoder: the intra edge filter copied the whole edge to read a 5-wide window — LANDED ✅ 2026-09-09 (byte-identical, -0.32 % at 1 MP)

Record: `benchmarks/encoder_intra_edge_window_2026-09-09.md` (+ two band TSVs).
KB-PERF-11's finding one function over, found by following its own named
follow-up.

- **THE DEFECT.** `intra::edge::highbd_filter_intra_edge` opened with
  `let mut edge = [0i32; 129];` plus an `sz`-element copy — **516 bytes of
  zero-init and an O(sz) copy per call** so that a **5-tap** filter could read
  original samples while writing `p` in place. **85.7 ms vs
  `av1_filter_intra_edge_sse4_1` 16.7 ms (5.1x)**.
- **THE FIX: carry the 5-wide window.** Invariant — on entry to iteration `i`,
  `w[j] == original p[clamp(i-2+j)]`. It holds at `i = 1` (nothing written yet);
  the shift re-establishes it because old `w[j+1]` names the same index as new
  `w[j]`; and the incoming `w[4]` reads `p[clamp(i+2)]`, whose index is `>= i`
  and so unwritten — including at `i == sz-1`, where the clamp makes it `i`
  itself and the read precedes the write. **The clamp is why the window is
  positional and shifted rather than addressed by offset**: at `i = 1` both
  `j = 0` and `j = 1` collapse onto index 0, so a "`k == i-1` -> slot 1,
  `k == i-2` -> slot 2" mapping is simply wrong.
- **BITE PROOF, ASYMMETRIC.** Dropping the `w[4]` refresh — staleness being the
  one way a carried window can diverge from C's flat copy — fails exactly **3**
  tests while **42** stay green, **including
  `edge_diff::filter_intra_edge_byte_identical`, the lowbd twin deliberately
  left untouched**: same file, same shape, unmodified, still passing.
- **MEASURED** (rotated arms, same-binary null, byte-identical 39,694 B):
  **-0.32 % at 1024x1024 (13/16, p=0.0213)** and **-0.29 % at 512x512 (18/24,
  p=0.0227)**; nulls +0.03 % / -0.05 %. The 1024 band was run at 16 rounds
  rather than 12 because the 512 band put the effect near the floor (KB-PERF-6's
  corollary: size the band to THIS effect). **This is the smallest of the four
  levers this cycle and is reported as such** — p ~ 0.02, not p < 0.001.
- **METHODOLOGICAL CAVEAT ON THE RATIO CHAIN — read before quoting digits.**
  `2.478x -> 2.464x -> 2.440x -> 2.424x` is **four ratios each measured against a
  C median from its own band**, and the C arm drifts: **4181.9 / 4140.9 /
  4162.8 / 4188.5 ms this session, a 1.15 % spread**. Each ratio therefore
  carries ~**±0.6 %** from the reference alone and cross-band ratio differences
  under ~0.015 are not resolvable. **The paired port-vs-port deltas are the solid
  numbers.** In this band the ratio reads 2.430x -> 2.424x, LOWER than
  KB-PERF-11's closing 2.440x purely because that band's C reference was 0.6 %
  faster — nothing regressed.
- **Not covered:** the **lowbd `filter_intra_edge` twin** has the identical shape
  and is untouched (deliberately, to keep the bite proof asymmetric); it does not
  appear in this profile because the encoder is u16-through at every bit depth,
  so it is a decoder-side item. `highbd_upsample_intra_edge`'s `[0i32; 19]` is
  76 bytes and measured negligible.
- **Gates:** `just gate-landing` green in full — `test-next` **1503/1503**,
  `test-next-scalar` **1503/1503**, `census-gate` 4/4, `test-whereat` 4/4.
...
Record: `benchmarks/encoder_hadamard_arrays_2026-09-09.md` (+ two band TSVs).
Found by drilling into the rd-driver class of
`benchmarks/encoder_x86_reprofile_1024_2026-09-09.md` (+1023 ms / 16.1 %,
never profiled below class level).

- **THE FIRST RESULT IS A LEVER THAT DOES NOT EXIST, and §14 is why.** Drilling
  in produced two named counterparts with huge ratios — `intra_model_rd_y`
  **342.8 ms vs C's `intra_model_rd.constprop.0` 23.4 ms (14.6x)** and
  `txfm_rd_in_plane_intra` 318.0 vs `av1_txfm_rd_in_plane` + `block_rd_txfm`
  84.9 (3.7x). **Neither is real.** The port has **no `subtract` and no `satd`
  symbol at all** in the profile while C has `aom_subtract_block_sse2` 25.5,
  `_avx2` 15.1, `av1_subtract_block` 7.1, `av1_subtract_txb` 5.0,
  `aom_satd_avx2` 11.7, `av1_quick_txfm` 5.0 — those callees are INLINED on the
  port side and DISPATCHED on C's, so the self-costs are not comparable and a
  lever costed off that row is costed off C's dispatch overhead. **Check whether
  a symbol is an inlining sink before costing anything off it** — KB-PERF-6
  recorded this for a closure; this is the same trap for an ordinary function.
- **What survived the check** is the one member of that neighbourhood that is a
  separate symbol on BOTH sides: `hadamard_col8` 126.0 + `hadamard_8x8` 39.2 +
  `hadamard_4x4` 33.0 + `hadamard_16x16` 18.6 + `hadamard_32x32` 4.1 =
  **220.9 ms against C's ~31.8 ms (6.9x)**. `aom-dsp/src/dist/hadamard.rs` had
  no SIMD path of any kind — KB-PERF-6's shape.
- **WHY THIS IS NOT A SIMD LANDING.** A 2-D Hadamard is two passes with a
  transpose between; only one pass is lane-parallel at a time whichever lane
  assignment you pick (worked out in the record), and libaom's SSE2 resolves it
  with `_mm_unpacklo_epi16`-family shuffles. **magetypes has no shuffle, permute
  or unpack in 0.9.28 or 0.9.29 — only `blend`** (verified for KB-PERF-8's
  gather question, re-checked here), so an in-register 8x8 transpose is not
  expressible and routing it through memory costs most of the win.
- **THE ACTUAL COST WAS THE SIGNATURE.** `hadamard_col8` carries 24
  `wrapping_add`/`wrapping_sub` and took **126 ms — more than the three 2-D
  drivers combined**. It was `fn hadamard_col8(src: &[i16], off, stride)` reading
  `src[off + k*stride]`: a SLICE parameter means a bounds check per element
  against a runtime length — **128 bounds-checked strided loads per 8x8 block**
  (sixteen calls), an `[i16; 8]` returned by value into a `copy_from_slice`, and
  a runtime length that stops LLVM keeping intermediates in registers or
  vectorizing the caller. The fix is a signature, not an algorithm: the column
  kernels take already-gathered `[i16; 8]` / `[i16; 4]`, the drivers read rows
  into `[[i16; 8]; 8]` with one bounds check per row, and every later index is
  into a fixed-size array.
- **THE TRAILING TRANSPOSE IS FUSED, NOT DROPPED — and saying so is KB-12.**
  C writes `buffer[idx*8+k] = A[idx][k]`; its second pass reads
  `buffer[idx + k*8]`, which in that layout is `A[k][idx]`, so
  `B[idx] = col8([A[k][idx] for k])`; the final *"Extra transpose to match SSE2
  behavior"* (`avg.c:232-236`) is `coeff[i*8+j] = buffer2[j*8+i]` = **exactly
  `B[j][i]`**. Emitting `B[j][i]` in place IS the transpose. The derivation is
  written out at the function, because KB-12 is the standing proof that this
  transpose going missing moves the `eob` and NOTHING else (rate, distortion and
  skippability are all order-invariant) and reads as a genuine RD near-tie for
  four localization passes.
- **BITE PROOF, ASYMMETRIC.** Perturbing the fused transpose in `hadamard_8x8`
  alone (`b[j][i]` -> `b[i][j]`) fails
  `hadamard_diff::hadamard_satd_byte_identical` while
  `highbd_hadamard_diff::highbd_hadamard_satd_byte_identical` — a separate,
...
- **MEASURED** (two sha256-distinct binaries from one tree, arms ROTATED per
  ...
  | cell | base | new | paired | rounds | p | null |
  |---|---:|---:|---:|---:|---:|---:|
  | 1024x1024 cq27 s0 | 10301.7 ms | 10204.1 ms | **-0.94 %** | 12/12 | 0.0005 | +0.08 % (p=0.39) |
  | 512x512 cq27 s0 | 3688.0 ms | 3660.1 ms | **-0.74 %** | 18/20 | 0.0004 | -0.08 % (p=0.50) |

  **Ratio 2.488x -> 2.464x** (C median 4140.9 ms). **§14 at 2.4x**: the class row
  said ~190 ms addressable, the delta is ~97 ms — the right order, and the reason
  is not mysterious, since this removed the INDEXING around the butterflies and
  not the butterflies.
- **Not covered:** the **highbd** Hadamard path (`highbd_hadamard_8x8/16x16/32x32`,
  `i32` domain) has the identical slice-parameter shape and was deliberately left
  alone so the bite proof stays asymmetric — it is the obvious follow-up. One box,
  one content class, `--cpu-used 0`, x86-64.
- **Gates:** `just gate-landing` green in full — `test-next` **1503/1503**,
  `test-next-scalar` **1503/1503**, `census-gate` 4/4, `test-whereat` 4/4.

### KB-PERF-9 — Encoder: the forward transform COLUMN pass had no 4-wide arm, so every 4-wide forward transform ran the driver's SCALAR per-column loop — LANDED ✅ 2026-09-09 (2.521x -> 2.478x at 1024x1024, byte-identical)

Record: `benchmarks/encoder_fwd_col_4wide_2026-09-09.md` (+ two band TSVs).
Found by the 1024x1024 re-profile that opens
`benchmarks/encoder_x86_reprofile_1024_2026-09-09.md`.

- **THE DEFECT.** `try_fwd_col_pass` opened with a flat `if col_n % 8 != 0 {
  return false; }`, so any transform whose WIDTH is 4 declined the vector column
  pass and fell back to `fwd_txfm2d_core`'s scalar per-column loop. **The INVERSE
  column pass in the same file has had the arm since the 2026-07-17 AVX2
  landing** — `if col_n % 8 != 0 && !(col_n == 4 && half_batch_pays(row_n))` —
  and `half_batch_pays`'s own doc says *"x86-64 always says yes: the 4-wide arms
  are the shape the 2026-07-17 AVX2 landing measured and kept"*. The forward twin
  simply never got it. Cost on a 1 MP frame: **~294 ms/encode of scalar 1-D
  forward kernels** (`av1_fadst4` 65, `av1_fdct4` 54, `av1_fadst16` 47,
  `av1_fdct16` 46, `av1_fadst8` 42, `av1_fdct8` 41).
- **WHY A HALF-FILLED BATCH PAYS HERE AND DID NOT IN KB-PERF-3 — the KB-PERF-5
  lesson, third instance.** *Same shape, opposite verdict, because the baseline
  differs.* KB-PERF-3's half-idle `i16x16` competed against a full `i32x8` (both
  256 bits, so nothing changed) and measured null; here the competition is a
  SCALAR loop, so 4 live lanes is still ~4x. Both ends also stay CONTIGUOUS at
  4-wide, which is the inverse COLUMN pass's shape and not the row pass's — a
  distinction this file already prices, since `try_fwd_row_pass` records that
  `col_n < 8` degrades its loads to a gather and measured **+9.8 %**
  (*"gathers cost; scatters don't"*).
- **BIT-EXACT BY CONSTRUCTION, no numeric bound.** The idle lanes are zero and
  stay inert end to end: the 1-D kernels are linear, `shl_clamp64v`/`rshiftv` map
  0 to 0, and only `active` lanes are stored. The `lr_flip` store index needed no
  new case either — lane `j` holds source column `cg + j`, landing at
  `col_n - 1 - (cg + j)`, a descending run based at `col_n - cg - active`, which
  is the 8-lane arm's own expression with `active` for 8.
- **BITE PROOF, ASYMMETRIC.** Swapping two lanes in the new 4-wide non-flip store
  — the only perturbation reachable SOLELY through the new arm — fails
  `txfm2d_differential_fuzz` (vs the real exported C) and
  `txfm2d_simd_perm_diff::txfm2d_simd_equals_scalar_at_every_permutation` while
  **all 10 inverse tests stay green**. So the arm is genuinely reached and the
  green is not vacuous.
- **MEASURED** (two sha256-distinct binaries from one tree, arms ROTATED per
  round, a same-binary null in every band, byte-identical 39,694 B output):

  | cell | base | new | paired | rounds | p | null |
  |---|---:|---:|---:|---:|---:|---:|
  | 1024x1024 cq27 s0 | 10541.2 ms | 10362.6 ms | **-1.63 %** | 10/10 | 0.0020 | -0.15 % (p=0.11) |
  | 512x512 cq27 s0 | 3778.6 ms | 3712.2 ms | **-1.76 %** | 20/20 | <0.0001 | -0.19 % (p=0.82) |

  **Ratio 2.521x -> 2.478x** at 1024x1024 (C median 4181.9 ms). ~172 ms of the
  ~294 ms of scalar forward kernels removed — the right order, since the vector
  path is not free and reaches only the COLUMN pass.
- **NOT COVERED, precisely.** The column kernel spans `row_n` and the row kernel
  spans `col_n`, so the arm reaches exactly the WIDTH-4 transforms: TX_4X4 /
  TX_4X8 / TX_4X16 column passes are fixed; the forward ROW pass is **still
  scalar whenever `row_n == 4`** (TX_4X4, TX_8X4, TX_16X4), and must not be
  assumed symmetric — its loads from row-major `buf` are strided, i.e. the gather
  case this file has already measured losing. **aarch64 unmeasured**
  (`half_batch_pays` declines `row_n == 4` there, same policy as the inverse).
- **Gates:** `just gate-landing` green in full — `test-next` **1503/1503**,
  `test-next-scalar` **1503/1503**, `census-gate` 4/4, `test-whereat` 4/4.
...
The step KB-PERF-6's own closing note named (*"the next step is therefore not
wider lanes but register blocking ... folds pairs of pixels ... so `H` is touched
once per tile instead of once per pixel"*), sized by its 2026-09-08 correction at
**446 ms on a 1 MP frame — the largest loop-restoration symbol in the profile and
the worst kernel ratio anywhere in it (8.2x vs `compute_stats_win7_avx2` + win5 +
`_c` at 54.4 ms).**

- **THE FIX.** `acc_stat_line_impl` folds **FOUR** adjacent source columns before
  each `H` read-modify-write, with a new `gather_window_quad` that spans `win + 3`
  columns instead of `4 * win` (70 plane loads at win7 against 196, since the four
  windows overlap in all but three columns). Per 8 `H` elements the inner loop goes
  from 4 x (load y, mul, load H, add, store H) to (4 x load y, 4 x mul, 3 add, load
  H, add, store H) — 20 ops to 13, and **one quarter of the `H` traffic**.
- **BIT-EXACT, and for a WEAKER reason than the tier it replaces — stated at the
  site rather than glossed.** KB-PERF-6's form reassociated nothing at all (lanes
  were elements of one `H` row, so every accumulator saw the scalar tier's exact
  product sequence). This one DOES reassociate: `h += p0; h += p1; h += p2; h += p3`
  becomes `h += ((p0 + p1) + p2) + p3`. Integer addition is associative — over `i32`
  it is associative even on overflow, since wrapping addition is a group operation —
  so every accumulator ends at the same value, and the products themselves are
  untouched. The scalar tier stays verbatim as the reference; `pick_diff` compares
  BOTH tiers against the real exported C.
- **libaom's own instruction is NOT available here, and that is a lockfile fact
  worth recording.** `acc_stat_win7_one_line_avx2` folds PAIRS with
  `_mm256_madd_epi16`; the workspace pins **magetypes 0.9.28**, whose `i16x16` has
  no `madd_adjacent` (it lands in **0.9.29**), and `aom-dsp` is
  `#![forbid(unsafe_code)]` so the intrinsic cannot be hand-written. **A worker
  report claiming madd had to be hand-written per-arch was CORRECT for the pinned
  version and was "corrected" against 0.9.29 by mistake — check `Cargo.lock`, not
  the newest crate in the registry, before deciding an op exists.** It did not
  matter: KB-PERF-6 had already measured that the 16-bit multiply is not the limit
  and the per-element `H` RMW is, so folding in `i32` attacks the measured
  bottleneck and the lane width does not.
- **MEASURED** (two binaries from one tree, arms interleaved and ROTATED, a
  same-binary null arm in every band, byte-identical output on every arm):
  ...
  The 192x192 base arm reproduces the published 2.542x to four digits, so the
  deltas are read against a live control rather than a remembered number.
- **ATTRIBUTED BY SYMBOL, because a wall delta is not a symbol delta — the exact
  mistake KB-PERF-6's correction blockquote records.** On the 1024x1024 cell
  `acc_stat_line_impl_v3` is **4.01 % -> 1.90 %** of the profile, i.e.
  **433 ms -> 201 ms**, against a wall delta of -210 ms (consistent within
  sampling). Every other `restore::` symbol is unmoved: `calculate_intermediate`
  2.41 -> 2.45 %, `pixel_proj_error` 1.99 -> 1.86 %, `wiener` 1.73 -> 1.74 %. So
  the kernel is **2.2x faster** and its ratio to C's **8.2x -> ~3.7x**, and it is
  **no longer the largest loop-restoration symbol**.
- **THE LEVER IS BIGGER AT REAL IMAGE SIZE (-2.00 % vs -1.68 %), and that is the
  point of running both cells.** At 192x192 the bd8 u16 planes are ~108 KiB and sit
  in L2, so a traffic-reducing lever is structurally under-measured there — the same
...
- **WHAT IS NOW THE LARGEST LR SYMBOL: `calculate_intermediate` (2.45 %), and its
  ...
  over its 256 inputs) — a different kind of change, and one that must be proven
  over the whole domain rather than argued.
- **AVX-512 is NOT a tier addition, checked rather than assumed.**
  `magetypes/src/simd/impls/x86_v4.rs` implements only the 512-bit-wide backends
  (`I32x16Backend`, `I16x32Backend`, `U8x64Backend`, ...) and no 256-bit ones, so
  adding `v4` to an existing `define(i32x8)` kernel's tier list cannot compile; a
  512-wide kernel is a separate body.
- **Untouched and named:** `compute_stats_highbd` (bd10/12, a separate i64 loop);
  the 16-bit multiply half of libaom's fold (needs magetypes >= 0.9.29 or an
  equivalent widening multiply-add); and the loop-restoration stage as a whole is
  still **+1406 ms of the 1024x1024 gap** before this landing, so this is one lever
  inside it, not the stage.
- **Gates:** `just gate-encode` exit 0, **197/197 test results ok**; `pick_diff` +
  `pick_search` green against the real exported C in BOTH dispatch modes.
