...
- **KB-69** — Encoder: the KB-50 peak-memory estimate under-stated the measured peak up to 3.2x — `e1a97fe`'s per-leaf payload retention (+ the per-leaf tree outcomes) is a per-PIXEL term the single padded-sample model had no slot for (heaptrack: 160 of 190 MB at 1 MP s0); model re-fitted speed/chroma/thread-aware, 22/22 cells bounded, worst slack 4.84x — FIXED ✅ 2026-09-24; the 5x memory growth itself (39 -> 218 MB at 1024 s3) is measured, attributed and OPEN as a product cost
