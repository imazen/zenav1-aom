...
| ~~**`PREREQ-AOM-STANDALONE` — no standalone encode path: `port_encode*` takes a C libaom bootstrap BY SIGNATURE**~~ **LARGELY CLOSED 2026-09-02** — `aom_encode::key_frame::encode_key_frame` (`crates/aom-encode/src/key_frame.rs`) emits a self-contained temporal unit (TD OBU + PORT-AUTHORED sequence-header OBU + `OBU_FRAME`) with **no C bytes in the path**, gated by `aom-encode/tests/self_contained_key_frame.rs`: **225/225 cells byte-identical to real aomenc** (up from 186/186 — see the 2026-09-03 CORRECTION + EXTENSION paragraph below; incl. all four CDEF x loop-restoration combinations, `--cpu-used` 0..=9 and MANDATORY multi-tile at speeds 0..6; real aomenc's ALLINTRA default is CDEF OFF + restoration ON per `av1_cx_iface.c:3067`, byte-gated at every speed) across cq 0..63 (step 5, plus a 1..19 step-2 low-q arm), {mono, 4:2:0, 4:2:2, 4:4:4} x bd {8, 10, 12}, 16x16..512x512, 12 crops/partial-SB sizes incl. 1x1, and 5 content classes; both the real C decoder and `aom-decode` decode the port's own stream to the pixels real aomenc's stream decodes to. Each of the four listed dependencies is closed by name: (1) `derive_sequence_header` + `aom_encode::seq_level` (`set_bitstream_level_tier`); (2) `derive_frame_header` from `rc::base_qindex_from_cq` + `av1_get_tile_limits` + `pick_filter_level` + a new `txb_split_count` for the `tx_mode` flip; (3) the screen-content decision is now DRIVEN by `screen_detect::estimate_screen_content_antialiasing_aware` (and the same landing fixed a real bug in BOTH callers: C reads the 8-ALIGNED `y_width`/`y_height`, not the crop); (4) `temporal_delimiter_obu` + `sequence_header_obu` + `assemble_obu_frame_single_tile`. **Still open, named**: SB128 has NO config knob at all (never refused BY NAME — it is simply never producible, since `KeyFrameConfig` has no field for it); explicit `--tile-columns`/`--tile-rows` are not exposed either (only the mandatory multi-tile split is); `av1_determine_sc_tools_with_encoding` is unported (see the 2026-09-03 paragraph — now adversarially probed, not merely asserted-safe); `aom-bench`'s own `port_encode*` still takes a bootstrap by signature (unchanged — the standalone path is a separate entry point). **CORRECTION 2026-09-03**: the "four cells stay pinned-divergent" sentence that stood here (132x132/196x196/260x260/261x261) was STALE — those four were the tile-clamp bug (see the second issue-#15 comment / the "real shell bug" STATUS.md entry) and were already CLOSED, moved into the hard-pass sweep, before this paragraph was last edited; re-verified 2026-09-03 by reading `open_divergences_are_pinned()` at HEAD, which held exactly 3 pins (`PIN_cdef_speed4`, `PIN_4160x64_multitile_speed9`, `PIN_256x256_speed7`) before this session, not those four. **EXTENSION 2026-09-03** (commit `65ffb75d`): crossed bit-depth x speed x tile-count, which neither axis B (bd, speed-0-only) nor axis I (multi-tile, bd8-only) had crossed before — MEASURED new divergences, now 4 more pins (7 total): bd10 single-tile diverges ONLY at speed 4-5 (passes 1,2,3,6); bd10 multi-tile diverges at speed 1-5 (passes 6 only) — the OPPOSITE band; bd12 single-tile diverges at speed 1-3 (passes 4,5,6) — the MIRROR of bd10's single-tile band; bd12 multi-tile diverges at EVERY speed 1-6, no passing speed at all. Single-tile pins are `TilePayloadAndHeader` (the loop-filter level moves too, since `pick_filter_level` reads the port's own recon); multi-tile pins are `TilePayloadOnly` (every header field, including loop-filter, agrees — only the entropy-coded tile payload differs). This is a NEW, narrower-characterized divergence class, related to but not proven to share a root cause with the pre-existing `HBD_OPEN`/`b10_64` pin below (that pin's harness is bootstrap-driven; this shell derives its own headers). Also added: a 25-cell regression probe at the exact GEOMETRY of the historical "14 tiny" `av1_determine_sc_tools_with_encoding` poison-panic class (PARITY.md C3, zenmetrics `avifaom_round3_2026-08-30_open.tsv`, `8468.scale59x128.png`) — this shell does NOT inherit that panic (25/25 byte-exact); and two adversarial differential probes (105 cells, 0 divergences, incl. a checker-patch crossover sweep bracketing the base detector's own threshold from both sides on a small 64x64 frame) specifically targeting whether `av1_determine_sc_tools_with_encoding`'s absence is reachable within this shell's envelope — not found, across three independently-designed content shapes. Full method + every measured number: `crates/aom-encode/tests/self_contained_key_frame.rs` doc comments on axes J/J2/K/K2 and the two `probe_sc_tools_trial_gap_*` tests. ORIGINAL ENTRY BELOW, unedited: **`PREREQ-AOM-STANDALONE` — no standalone encode path: `port_encode*` takes a C libaom bootstrap BY SIGNATURE** (registered 2026-09-02 by zenmetrics `benchmarks/avif_doe_plan_2026-09-01.md` §16.4; owner = THIS repo) | `AomBenchCase::port_encode(&self, bootstrap: &[u8])` (`crates/aom-bench/src/lib.rs:1150`) and `port_encode_with(&self, bootstrap: &[u8], knobs: &ToggleKnobs)` (`:1176`) cannot be called without a stream a C encoder already produced, so **the port cannot emit a self-contained AV1 stream at any speed or depth today.** Measured 2026-09-02, four distinct dependencies (all in `port_encode_full`, `:1195`ff): **(1) no sequence-header emitter** — `walk_obus(bootstrap)` finds `OBU_SEQUENCE_HEADER` and `read_sequence_header_obu` (`:1203-1210`) supplies every sequence-level geometry/tool field the encode runs on (`max_frame_width/height`, `sb_size_128`, `enable_cdef`/`_restoration`/`_superres`, `force_screen_content_tools`, colour config, frame-id, order-hint, operating points, timing/decoder-model) into the `FrameHeaderObu` cfg (`:1232`ff). **(2) frame-header fields are PARSED, not DERIVED** — the bootstrap's `OBU_FRAME` (`:1211-1216`) goes through `read_uncompressed_header` (`:1313`, `:1328`) to produce `p`, which carries `base_qindex`, the quant deltas, tx mode and the rest; nothing maps a `--cq-level`/config to a header. **(3) the screen-content decision comes from C, though the DETECTOR is already ported** — `aom_encode::screen_detect::estimate_screen_content_antialiasing_aware` runs (`:1904`) but is only **asserted equal** to the bootstrap-parsed `p.allow_screen_content_tools` (`:1914`); the value actually threaded into the search is `p.allow_screen_content_tools` (`:1720`, `:2044`, `:2069`). So the detector is a differential gate here, not the driver — and the assert's own message names the remaining un-ported C arm, `av1_determine_sc_tools_with_encoding` (`encoder.c:3312`, the two q>=244 trial encodes, live on allintra below speed 8 — the same arm behind the 35 tiny cells registered in PARITY.md C3). **(4) the output is a bare frame OBU PAYLOAD, not a stream** — `assemble_frame_obu_payload_single_tile` / `assemble_multitile_frame_obu_payload_derived` (`:2487`, `:2494`) do re-serialize the frame header from `p` (the multi-tile path notes *"nothing is spliced from the bootstrap"*), but there is no temporal-unit / OBU framing and no sequence-header OBU on the way out. **Why it is queued rather than merely noted:** under the workspace imazen-only rule a model that tunes zenav1-aom may not be trained on cells C partly encoded. DOE arm **A3 is BLOCKED** on this, and the A0 aom cells already drained are re-labelled **port-parity evidence** rather than tuning data (nothing re-run, nothing deleted — DOE §16.4 table). The svt arm carries the DOE meanwhile. **Done** = the port emits a decode-verifiable self-contained OBU stream across its supported depths/speeds, validated by its own decode + a `zenavif-parse` read-back; C parity stays a SEPARATE differential gate (this is not a request to stop comparing against libaom — DOE §16.5). Admission context: the T4 `HBD_OPEN` pin below (bd10/bd12 x `--cpu-used` 1..6, pinned-divergent, no refusal) is why a standalone path still needs its own byte/decode gate rather than inheriting the harness's C compare. Tracking issue: https://github.com/imazen/zenav1-aom/issues/15 |
