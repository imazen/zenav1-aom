...
| `SCREEN_S4_S5_OPEN` (no gate — the localizer runs on dumped planes) | screen-detected frames at `--cpu-used` 4/5: 1280x800 cq44 s4 (+10 B), 1920x1080 cq57 s4 (+10 B), 1920x1080 cq6 s4 (−33 B), 1280x800 cq25 s5 (−5 B), 1920x1080 cq32 s5 (−123 B) — measured 2026-08-30 after KB-41 roots #14-#17 on `~/tmp/aomplanes_s4`/`_s5` (the s6 planes with `speed` rewritten); 6/9 s4 and 1/3 s5 cells are exact, so these are RD near-ties/prunes past the four roots, not a structural gap | first-syntax-diff + the C/port per-txb probes (the #15-#17 method); the wave's cpu-4 screen cells hit these (byte-refused, re-queued) |
