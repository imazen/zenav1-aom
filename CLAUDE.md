...
| (5) match the RD of C | byte identity is the strongest available evidence and holds on 549/549 standalone cells — including the entire former `HBD_OPEN` band (KB-61) — and on every speed-0 pinned band: `MONO_S0_OPEN` (KB-27), `SPEED0_1080P_OPEN`/`HD_HBD_OPEN` (KB-38's residual) and the mono content-near-ties all closed 2026-09-13 as KB-62 (the AB-reuse clone's stale `tx_type_map`; C aliases `xd->tx_type_map` to the ctx array so the copied map is post-dry-run). Still-open pinned divergences, each measured/attributed/bounded: the size-axis finding-B cells, the SCM trial gap, and cpu-8 photo rows needing content not in-repo. (`NONRD_CQ63_OPEN` also re-measured clean — KB-58's per-SB-qindex plumbing had closed it; the pin sat stale since `ae93253`.) |
