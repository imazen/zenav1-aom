...
| (6) sensible conversion + wiring + testing of all of the C encoder | **Palette + IntraBC were NOT wired into the shipping path until 2026-09-10** — `encode_key_frame` ran neither search on any frame; the 427/427 byte gate's oracle hardcodes `--enable-palette=0 --enable-intrabc=0`, so it was blind by construction (KB-42's shape on a new axis). Now gated by `screen_content_tools_byte_match_real_aomenc` (54/54, matched oracle), and that gate found a header-length stream-corruption bug on its first run (constant +3 B over a byte-identical tile payload). Bounded divergence: IntraBC declines at coded-lossless. The SCM trial encode is unported and MEASURED to be a divergence, not a refusal (60/60 tiny screen cells encode). The bd12 dispatch-tier disagreement is closed and gated. Six features are ported and byte-gated but still harness-only (README table). Full row: `docs/CLAUSE_STATUS_LOG.md`. |
