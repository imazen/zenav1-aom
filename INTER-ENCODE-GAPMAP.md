...
|---|---|---|
| ~~`encoder/firstpass.c`~~ | ~~43~~ **7 left** | 2-pass (`--passes=2`) — see §3.1 |
| `encoder/pass2_strategy.c` | 85 | 2-pass |
| ~~`encoder/tpl_model.c`~~ | ~~62~~ **11 left** | `lag>0` + TPL enabled — see §3.1 |
...
**59 C functions ported and gated.** Both files were "IN SCOPE, DEFERRED"
...
Gates: `crates/aom-encode/tests/tpl_model_diff.rs` (38/38),
`crates/aom-encode/tests/firstpass_diff.rs` (27/27), both green under the dev
and `test-fast` profiles.
...
| external-RC API only | 3 | `init_tpl_stats_before_propagation`, `tpl_store_before_propagation` and `trim_tpl_stats` are each guarded by `av1_use_tpl_for_extrc(&cpi->ext_ratectrl)` (tpl_model.h:420 — `ext_rc->ready && ext_rc->funcs.send_tpl_gop_stats != NULL`), at tpl_model.c:1416, :1573 and :2154. The port has no `ext_ratectrl` surface at all (`grep -rn ext_ratectrl crates/*/src` is empty), so the guard is false everywhere and these are unreachable. Same class as the external-partition hooks in §2.1. |
| **still missing, in scope** | **11** | named below |
...
**Still missing (11)**, all `static` bar one, all one connected subsystem —
the per-block TPL pass and the GOP driver that runs it. They need the
encoder's frame buffers, motion search and quantizer wired up, which is the
next chunk: `mode_estimation`, `motion_estimation`, `get_rate_distortion`,
`get_inter_cost`, `get_quantize_error`, `txfm_quant_rdcost`,
`init_mc_flow_dispenser`, `mc_flow_dispenser`, `init_gop_frames_for_tpl`,
`av1_tpl_setup_stats`, `av1_tpl_preload_rc_estimate`.
...
`accumulate_frame_stats`, `accumulate_mv_stats`, `get_bsize`,
`get_block_variance_fn`, `highbd_get_block_variance_fn`,
`get_prediction_error`, `highbd_get_prediction_error`,
`get_prediction_error_bitdepth`, and the arithmetic half of
`update_firstpass_stats`.

**Still missing (7)** — the per-block first pass itself:
`firstpass_intra_prediction`, `firstpass_inter_prediction`,
`first_pass_motion_search`, `first_pass_intra_pred_and_calc_diff`,
`first_pass_predict_intra_block_for_luma_plane`, `av1_first_pass`,
...
shims, it reports **33 unmatched for tpl_model.c and 29 for firstpass.c**,
against the 11 and 7 above. The difference is two things the tool cannot see,
and neither is a gap:

* **renames.** `av1_twopass_zero_stats` is `FirstpassStats::zero`,
  ...
  `av1_init_tpl_txfm_stats` is `TplTxfmStats::init`,
  `get_block_variance_fn` is `block_variance_dims` (C returns a function
  pointer; the port returns the dimensions those four kernels differ by),
  `update_firstpass_stats` is `FirstpassStats::from_frame_stats`, and so on.
* **the four out-of-queue classes** above — compiled-out, mechanism,
  external-RC-only — which are `MISSING` to a name matcher and are not work.

The tool's own header says a miss can be a rename; on these two files that is
most of the misses, so the counts above are the ones to use. **Verify a row
before believing either direction**, which is what §3.1 is.
...
| `av1_enc_build_one_inter_predictor`, `av1_enc_build_inter_predictor_y`, `_y_nonrd`, `av1_build_inter_predictors_for_planes_single_buf`, `enc_build_inter_predictors` (reconinter_enc.c) | `av1_make_inter_predictor` → `av1_convolve_2d_facade` (common/convolve.c) — the ROUTING only. Every leaf it dispatches to is ported and gated (`convolve_2d_sr`, `_x_sr`, `_y_sr`, the four `dist_wtd_convolve_*`, `convolve_copy`), and the block-size-dependent filter selection that feeds them landed with them: `filter_table_id` + the `av1_interp_4tap` tables (`aom_dsp::convolve`), gated at tier 1c by `aom-dsp/tests/interp_filter_params_diff.rs`. What is left is the facade's four-way branch, `inter_predictor`'s `revert_scale_extra_bits`, and an `InterPredParams` to carry `conv_params`. The WARP_PRED arm additionally needs `av1_warp_plane`. |
