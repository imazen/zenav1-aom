...
# The captured NAME must allow capitals. libaom has CamelCase-tailed C
# functions — `av1_findSamples` and `av1_selectSamples` (mvref_common.c:1092,
# :1118), the WARPED_CAUSAL sample collectors, squarely in scope for inter.
# A lowercase-only class did not merely fail to MATCH them, it failed to SEE
# them: they were absent from the denominator entirely, so the file read as
# more complete than it is. Measured 2026-08-31.
DEF = re.compile(r'^(?:static\s+)?(?:const\s+)?[A-Za-z_][\w \*]*?\b([A-Za-z_][A-Za-z0-9_]*)\s*\([^;]*?\)\s*\{', re.M)
