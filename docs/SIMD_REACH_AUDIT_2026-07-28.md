...
| **cdef** — `cdef_find_dir` (partial sums) | `define(i16x8, u16x8), v3, neon, wasm128, -scalar` | **YES** (landed 2026-07-28, F6 half 1) | `cdef/simd.rs::cdef_find_dir_partials_impl`; entry routes on a CHECKED value predicate, scalar tier declines | **YES** — `tests/cdef_find_dir_simd_diff.rs`, 25 permutations, `simd_perms >= 1` **and** a kernel-accept liveness unit test |
| **dist** — `hadamard_*` / `highbd_hadamard_*` (SATD) | *(none)* | N/A — pure scalar, no SIMD tier | source read: `dist/hadamard.rs` | N/A. See **F6** |
...
### F6 — MEDIUM (perf). Two named hotspots have no SIMD tier at all — `cdef_find_dir` DONE 2026-07-28, SATD still open
...
**`cdef_find_dir` half — DONE 2026-07-28.** Its partial-sum half now has ONE
`#[magetypes(define(i16x8, u16x8), v3, neon, wasm128, -scalar)]` kernel
(`cdef/simd.rs::cdef_find_dir_partials`); the cost fold, the argmax and its normative
tie-break stay in a single shared copy (`cdef::find_dir_cost_fold`). i16 lanes are exact
under a **checked** per-call value predicate (`cdef_find_dir_simd_eligible`:
`(px >> coeff_shift) <= 255`, which is the whole domain the decoder produces), so the
entry is bit-identical to the scalar port for every input with no range assumption; outside
it, and under the scalar pin, the transcribed `cdef_find_dir_scalar` runs. Measured
**−24.3 % wall** on `cdef::find_dir_08x08` (two-build alternation, medians of 5) against
untouched control rows at −0.27 %/+0.07 % in the same interleaved rounds —
`benchmarks/cdef_find_dir_simd_2026-07-28.md`. **Ir/decode not re-measured** (no callgrind
on the aarch64 box); the q32 share of this landing is not claimed.

The remaining shape gap is register residency: `cdef_find_dir_avx2` shifts LANES where this
kernel shifts the ADDRESS, and magetypes 0.9.27 has no i16 lane shift, cross-lane permute,
integer widen/narrow or madd (`interleave`/`transpose_8x8`/`cross_width` are f32-only) — so
that needs per-tier `#[rite]` primitives in the `prims.rs` Pattern-C shape. Chunk-2 plan
with the exact primitive list is in the benchmark record. **The SATD half is untouched.**
...
4. ~~**F6** — port `cdef_find_dir` to magetypes (~4.7% of q32 decode Ir, C has an AVX2
   version)~~ DONE 2026-07-28 (i16-lane kernel, −24.3 % wall on the bench row;
   `benchmarks/cdef_find_dir_simd_2026-07-28.md`). **Still open:** the SATD/hadamard
   family, and F6's chunk 2 (per-tier lane-shift/transpose primitives for register
   residency).
